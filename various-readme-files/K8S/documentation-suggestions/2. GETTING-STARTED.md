...
### Traffic Flow
```
User → httpd:80/443 → a3gw:8444/8445 → portals (8080-8082) → Backend APIs
```
...
└────────────────────────┬────────────────────────────────────┘
                ...
                ▼                 ▼            ▼
        ┌───────────────┐  ┌──────────┐  ┌─────────────┐
        │ Admin Portal  │  │CC Portal │  │Partner Port │
        │   :8080       │  │  :8081   │  │    :8082    │
        └───────────────┘  └──────────┘  └─────────────┘
                         │
                         ▼
...
→ **Go to [Path 1: Docker Compose](#path-1-docker-compose-fastest)**

### 🎯 I want to test Kubernetes deployment locally
→ **Go to [Path 2: Local Kubernetes](#path-2-local-kubernetes)**

### 🏢 I want to deploy to our corporate cluster
→ **Go to [Path 3: Corporate Kubernetes Cluster](#path-3-corporate-kubernetes-cluster)**
...
**Features:**
- ✅ Hot-reloading for portal changes
...
- ✅ No volume mounts (faster)
- ✅ Runs on ports 9080, 9443
...
#### ⭐ **Option 1: NodePort (Easiest - Recommended for Mac)**
...
**Cons:**
- ⚠️ Different ports than production (30080 vs 80)
...
**Cons:**
- ⚠️ Must manually start port-forwards
- ⚠️ Stops when terminal closes
...
**Cons:**
- ⚠️ Requires Ingress Controller
- ⚠️ Extra component to manage
...
├── vcp-adminportal/         # Admin portal (AngularJS)
├── vcp-ccportal/            # CC portal
├── vcp-partnerportal/       # Partner portal
...
├── values-local*.yaml       # Local variants
├── docker-compose_*.yml     # Docker Compose files
...
**Docker:**
- Port conflicts → See [README-Make.md](README-Make.md)
- Volume issues → Try `make fresh dev`

**Kubernetes:**
- Image pull errors → Check registry access and image tags
- Pod crashes → Check logs with `kubectl logs`
- Port conflicts → Use different NodePort values

**Corporate Cluster:**
- Image architecture → Must use AMD64 images with `-amd64` suffix
- Registry access → Ensure `docker login nexus.telenity.com` works
- Namespace access → Verify with `kubectl get pods -n stc-vcp-services`
