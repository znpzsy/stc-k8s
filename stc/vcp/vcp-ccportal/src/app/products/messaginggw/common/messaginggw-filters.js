(function () {
    ...
    /* Filters */
    angular.module('ccportal.products.messaginggw.filters', []);

    var MessagingGwFilters = angular.module('ccportal.products.messaginggw.filters');
    ...
    MessagingGwFilters.filter('MessagingGwEDRResultReasonFilter', function (MSGGW_EDR_RESULT_REASONS) {
        return function (context, code) {
            var reason = _.findWhere(MSGGW_EDR_RESULT_REASONS, {reason_context: Number(context), reason_code: Number(code)});
    ...
    MessagingGwFilters.filter('MessagingGwChargingEventFilter', function (MSGGW_CHARGING_EVENTS) {
        return function (state) {
            var chargingEventState = _.findWhere(MSGGW_CHARGING_EVENTS, {state: Number(state)});

            if (chargingEventState)
                return chargingEventState.text;
            else
                return state;
        };
    });
