(function () {
    ...
    angular.module('partnerportal.forgotpassword', []);

    var ForgotPasswordModule = angular.module('partnerportal.forgotpassword');
    ...
    ForgotPasswordModule.controller('ResetPasswordCtrl', function ($scope, $log, $state, $stateParams, $interval, $window, $translate, $filter, notification, CMPFService, UtilService, userAccount) {
        $log.debug('ResetPasswordCtrl');
        $log.debug('ResetPasswordCtrl userAccount from resolve: ', userAccount);
        ...
        $scope.prepareEmptyUserProfile = function () {
            ...
            //$log.debug("prepareEmptyUserProfile, empty user profile prepared with attributes: ", profile);
            return profile;
        };
        ...
        $scope.confirmPasswordReset = function (userAccount) {
            ...
            $log.debug("confirmPassReset, updated version of $scope.originalUserAccount to be sent", updatedAccount);
            ...
            CMPFService.updateUserAccount(updatedAccount).then(function (response) {
                ...
                if (response) {
                    // TODO: Not sure but body containing errorCode returns with 200 OK from time to time. Checks below can be edited in the future.
