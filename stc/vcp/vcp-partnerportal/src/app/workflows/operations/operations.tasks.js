(function () {
    ...
    WorkflowsOperationsTasksModule.config(function ($stateProvider) {
        ...
        $stateProvider.state('workflows.operations.tasks', {
            url: "/:taskStatus/:taskType/:removeId",
        ...
        }).state('workflows.operations.tasks-detail.offer-revshare', {
            url: "/offerrevshare",
            templateUrl: 'workflows/operations/operations.tasks.offers.revshare.detail.html',
            controller: 'WorkflowsOperationsTasksDetailOfferRevshareCtrl',
            resolve: {
                taskDetail: function ($q, task) {
                    return task;
                }
            }
    ...
    WorkflowsOperationsTasksModule.controller('WorkflowsOperationsTasksCommonCtrl', function ($scope, $log, $controller, $state, $stateParams, $uibModal, $translate, notification, Restangular,
                                                                                              WorkflowsService, WORKFLOWS_STATUSES, PartnerPortalNotificationListPromiseTracker) {
        ...
        $scope.approveTask = function (task){
            WorkflowsService.approveTask(task.id, "Revshare Price Accepted");
        ...
        $scope.deleteTask = function (task) {
            ...
            modalInstance.result.then(function () {
                ...
                else if (task.type === 'OFFER_CONTRACTED') {
                    cancelMethod = WorkflowsService.rejectTask;
                    itemId = task.id;
                }
        ...
        $scope.lazyLoad = function () {
            ...
            if ($scope.tasks && $scope.tasks.length < $scope.totalRecords) {
                WorkflowsService.getTasks(page, 100, queryStatus, taskType, PartnerPortalNotificationListPromiseTracker)
                    .then(function (ts) {
                            page++;
    ...
    WorkflowsOperationsTasksModule.controller('WorkflowsOperationsTasksCtrl', function ($scope, $log, $controller, $state, $stateParams, $translate, Restangular,
        ...
        var taskStatus = $stateParams.taskStatus;
        var taskType = $stateParams.taskType;
        var removeId = $stateParams.removeId;
        ...
        $scope.showDetail = function (taskType, taskId) {
            ...
            } else if (taskType === 'OFFER_CONTRACTED') {
                url = 'workflows.operations.tasks-detail.offer-revshare';
            }
    ...
    WorkflowsOperationsTasksModule.controller('WorkflowsOperationsTasksDetailOfferRevshareCtrl', function ($scope, $log, $controller, Restangular, UtilService, CMPFService, DEFAULT_REST_QUERY_LIMIT,
                                                                                                   taskDetail) {
        $log.debug("WorkflowsOperationsTasksDetailOfferRevshareCtrl");

        $controller('WorkflowsOperationsTasksCommonCtrl', {$scope: $scope});

        $scope.task = taskDetail;
        _.each($scope.task.objectDetail.attributes, function (attr) {
            $scope.task[attr.name] = attr.value;
        });
    });
