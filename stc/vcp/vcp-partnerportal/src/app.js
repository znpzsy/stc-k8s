(function () {
    ...
    var DEFAULT_STATE_NAME = 'partner-info.partner-profile';
    ...
    PartnerPortalApplicationModule.run(function ($rootScope, $log, $q, $timeout, $translate, $document, $http, notification, Restangular, SessionService, $sce, $state, $stateParams,
        ...
        // Gets organization name of the current logged in user
        $rootScope.getOrganizationLegacyId = function () {
            var organization = SessionService.getSessionOrganization();
            var legacyId = CMPFService.getProfileAttributes(organization.profiles, CMPFService.SERVICE_PROVIDER_LEGACY_ID_PROFILE) ? CMPFService.getProfileAttributes(organization.profiles, CMPFService.SERVICE_PROVIDER_LEGACY_ID_PROFILE)[0].LegacyID : null;
        ...
        $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
            $log.debug('$stateChangeStart event: ', event, toState, toParams, fromState, fromParams);
            ...
            if ($rootScope.isSessionValid()) {
                ...
                $log.debug("User profiles: ", userProfiles);

                if (userProfiles.length > 0 && userProfiles[0].EnforcePasswordChange) {
                    $state.go("resetpassword").then(function (state) {
                    ...
                    if (partner && partner.profiles && partner.profiles.length > 0) {
                        // Check profiles for allowed categories. If there is no profile attached to the provider, then invalidate the session.
                        // var providerCommonProfile = CMPFService.getProfileAttributes(partner.profiles, CMPFService.SERVICE_PROVIDER_COMMON_PROFILE);

                        if (toState.name === 'login' || toState.name === 'firsttimelogin.changepassword') {
                            var requiredPermissions = $state.get(DEFAULT_STATE_NAME).data.permissions;
                            if(AuthorizationService.checkPermissionList(requiredPermissions)) {
                                $state.go(DEFAULT_STATE_NAME, null, {notify: false}).then(function (state) {
                                    $rootScope.$broadcast('$stateChangeSuccess', state, null);
                                });
                            } else {
                                $rootScope.$broadcast('$stateChangeError', event, toState, toParams, fromState, fromParams);
                            }
                        } else {
        ...
        $rootScope.$on('$stateChangeError', function (event, toState, toParams, fromState, fromParams, error) {
            $log.debug('$stateChangeError event: ', event, toState, toParams, fromState, fromParams);
            notification({
                type: 'warning',
                text: $translate.instant('CommonMessages.InsufficientPermissions')
            });
