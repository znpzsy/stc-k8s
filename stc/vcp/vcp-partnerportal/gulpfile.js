...
var applicationName = 'vcp-partnerportal';
...
function hideVersions() {
    var jqueryVersion = "3.6.3";
    var angularVersion = "1.8.3";
    var hiddenVersion = "0.0.0";
    return [
        replace("jquery:\"" + jqueryVersion + "\"", "jquery:\"" + hiddenVersion + "\""),
        replace("\"" + jqueryVersion + "\"+", "\"" + hiddenVersion + "\""),
        replace("errors.angularjs.org/" + angularVersion + "/", "errors.angularjs.org/" + hiddenVersion + "/"),
        replace("full:\"" + angularVersion + "\"", "full:\"" + hiddenVersion + "\""),
        replace("angularVersion:\"" + angularVersion + "\"", "angularVersion:\"" + hiddenVersion + "\""),
        replace("VERSION=\"" + angularVersion + "\"", "VERSION=\"" + hiddenVersion + "\"")
    ] }
...
gulp.task('usemin-main', ['clean', 'config'], function () {
    return gulp.src(bases.src + '/*.html')
        .pipe(usemin({
            ...
            small_libjs: [ngAnnotate(), uglify(uglifyOptions), rev()].concat(hideVersions()),
            libjs: [ngAnnotate(), uglify(uglifyOptions), rev()].concat(hideVersions()),
            index_js: [ngAnnotate(), uglify(uglifyOptions), rev()].concat(hideVersions()),
            app_js: [ngAnnotate(), uglify(uglifyOptions), rev()].concat(hideVersions())
        }))
...
gulp.task('copy', ['copy-img', 'copy-i18n', 'copy-html5-js', 'copy-fonts']);
...
gulp.task('config', function () {
    return gulp.src('./src/app-config.json')
        .pipe(gulpNgConfig('partnerportal.config', {
            //constants: {}
        }))
        .pipe(gulp.dest('./src/'))
});
...
gulp.task('connect', ['fast-dist'], function () {
    return connect.server({
        ...
        middleware: function (connect) {
            return [
                connect().use('/partnerportal/i18n', serveStatic('i18n')),
                connect().use('/partnerportal/fonts', serveStatic('bower_components/font-awesome/fonts')),
                connect().use('/partnerportal/fonts', serveStatic('bower_components/bootstrap/fonts')),
                connect().use('/partnerportal/fonts', serveStatic(bases.src + '/css/google-fonts')),
                connect().use('/partnerportal/js', serveStatic(bases.src + '/html5')),
                connect().use('/partnerportal/img', serveStatic(bases.src + '/img')),
                connect().use('/partnerportal', serveStatic(bases.src + '/app')),
                connect().use('/partnerportal', serveStatic(bases.tmp))
            ];
        }
    });
});
