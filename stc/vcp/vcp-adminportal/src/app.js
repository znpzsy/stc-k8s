(function () {
    ...
    AdmPortalApplicationModule.run(function ($rootScope, $log, $q, $timeout, $translate, $document, $http, $window, notification, Restangular, SessionService, $sce, $state, $stateParams,
                                             IdleServiceFactory, CMPFService, UtilService, AuthorizationService, DateTimeConstants, CURRENCY, AdmPortalMainPromiseTracker, DEFAULT_REST_QUERY_LIMIT) {
        ...
        // Taken from DSP - Get the flag that shows if the user is BMS admin user.
        ...
        // DSP - Check the latest state and redirect there.
    ...
    AdmPortalApplicationModule.config(function ($translateProvider, RestangularProvider, $stateProvider, $urlRouterProvider, $logProvider, $qProvider,
        ...
        IdleProvider.setLocalStorageKey('vcp-ap-stc-sa');
        ...
        $translateProvider.storagePrefix('VCP_AP_STC_SA_');
        ...
        RestangularProvider.setRequestInterceptor(function (elem, operation, path, url) {
            if(operation === 'remove') {
                ...
                if(s.include(url, "/keyword-screening") || s.include(url, "/screeninglists")){
                    return elem
                }
                if(!s.include(url, "/pentaho/api")){
