(function () {
    ...
    var getDataArray = function (columns, object, isQuote, isEscapeHTML) {
        ...
        _.each(columns, function (column) {
            ...
            try {
                fieldValue = eval('object.' + column.fieldName);
            } catch (e) {
                fieldValue = '';
                column.filter = undefined;
            }

            if (!_.isUndefined(column.filter)) {
                var params = [], param;
                _.each(column.filter.params, function (param) {
                    try {
                ...
                var filterParams = (column.filter.params ? _.union([fieldValue], params) : [fieldValue]);
                fieldValue = filterFunc(column.filter.name).apply(null, filterParams);
            }
