(function () {
    ...
    ApplicationDirectives.directive('uiIndeterminate', function () {
        return {
            link: function (scope, el, attrs) {
                scope.$watch(attrs.uiIndeterminate, function (val) {
                    el[0].indeterminate = !!val;
                });
            }
        };
    });
    ...
    ApplicationDirectives.directive('lineLengthValidator', function() {
        return {
            require: 'ngModel',
            link: function(scope, element, attrs, ngModel) {
                ngModel.$validators.lineLength = function(modelValue, viewValue) {
                    if (!viewValue) {
                        return true; // Valid for empty string
                    }

                    var maxLength = parseInt(attrs.maxLineLength, 10) || 20; // Default to 20 if not provided
                    ...
                    // If all lines are within acceptable length, return valid
                    return true;
                };

                // Watch for max-line-length
                attrs.$observe('maxLineLength', function(newVal) {
                    if (newVal) {
                        ngModel.$validate();
                    }
                });
    ...
    ApplicationDirectives.directive('validEmail', function () {
        var re = /^(?=.{6,100}$)[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:(?:xn--)?[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{2,63})$/i;
        ...
        return {
            ...
            link: function (scope, el, attrs, ngModel) {
                ...
                ngModel.$formatters.push(function (v) { return normalize(v); });

                ngModel.$validators.validEmail = function (modelValue, viewValue) {
                    var v = modelValue || viewValue;
                    if (ngModel.$isEmpty(v)) return true; // leave "required" to ng-required
                    return re.test(v);
                };
            }
        };
        ...
        return {
            ...
            link: function (scope, el, attrs, ngModel) {
                ngModel.$parsers.push(function (val) {
                    ...
                    var ok = re.test(val.trim());
                    ngModel.$setValidity('validEmail', ok);
                    return ok ? val.trim() : undefined;
                });
            }
        };
    });*/
