(function () {
    ...
    SubscriptionManagementOperationsOffersModule.controller('SubscriptionManagementOperationsOffersCommonCtrl', function ($scope, $log, $controller, $q, $state, $filter, $uibModal, notification, $translate, UtilService,
                                                                                                                          ...
                                                                                                                          OFFER_BUNDLING_TYPES, OFFER_ELIGIBILITY_EVALUATION_POLICIES, OFFER_MAIN_OFFER_EVALUATION_POLICY,
                                                                                                                          DEFAULT_REST_QUERY_LIMIT, businessTypesOrganization, settlementTypesOrganization, serviceCategoriesOrganization, packageList) {
        ...
        var emptyCmpfObject = { profiles: [] }; // Empty object to be used in case of no data.

        var businessTypesOrganizationItem =  businessTypesOrganization.organizations[0] ? businessTypesOrganization.organizations[0]: emptyCmpfObject;
        var allBusinessTypes = CMPFService.getBusinessTypes(businessTypesOrganizationItem);
        $scope.businessTypes = [];

        var settlementTypesOrganizationItem = settlementTypesOrganization.organizations[0] ? settlementTypesOrganization.organizations[0]: emptyCmpfObject;
        ...
        $scope.OFFER_BUNDLING_TYPES = OFFER_BUNDLING_TYPES;
        $scope.OFFER_ELIGIBILITY_EVALUATION_POLICIES = OFFER_ELIGIBILITY_EVALUATION_POLICIES;
        $scope.OFFER_MAIN_OFFER_EVALUATION_POLICY = OFFER_MAIN_OFFER_EVALUATION_POLICY;
        $scope.organizationDictionary = {};
        $scope.servicePriceData = [];

        $scope.resetAllServiceAndOrganizationInfo = function (){
            $scope.serviceAndOrganizationData();
            $scope.resetPrices();
        }

        $scope.resetPrices = function (){
            if(!$scope.offer.offerBundlingProfile.BundlingEnabled || $scope.offer.offerBundlingProfile.BundlingOption !== 'MOBILY')
                return;
            $scope.remainingServicePrice = $scope.offer.xsmChargingProfile.MainPrice;
            $scope.servicePriceData = [];
            if($scope.selectedServices){
                for(var i = 0; i<$scope.selectedServices.length; i++){
                    $scope.servicePriceData.push(
                        {
                            name: $scope.selectedServices[i].name,
                            organization: $scope.organizationDictionary[$scope.selectedServices[i].organizationId],
                            price : 0,
                            percentage: 0,
                            serviceId: $scope.selectedServices[i].id,
                            organizationId: $scope.selectedServices[i].organizationId
                        }
                    )
                }
            }
        ...
        $scope.serviceAndOrganizationData = function (){
            $scope.offer.organization = undefined;
            $scope.offer.organizationId = undefined;
            $scope.offer.tempOrganization = undefined;
            $scope.offer.tempOrganizationId = undefined;
            $scope.selectedServices = undefined;
        }

        $scope.editPriceDistribution = function () {
            var modalInstance = $uibModal.open({
                templateUrl: 'partials/modal/modal.distribution.html',
                controller: 'DistributionModalInstanceCtrl',
                size: 'lg',
                resolve: {
                    servicePriceData: function (){
                        return angular.copy($scope.servicePriceData);
                    },
                    fullPrice: function (){
                        return $scope.offer.xsmChargingProfile.MainPrice;
                    },
                    remainingPrice: function (){
                        return $scope.remainingServicePrice === undefined || $scope.remainingServicePrice === null ? $scope.offer.xsmChargingProfile.MainPrice : $scope.remainingServicePrice;
                    }
                }
            });

            modalInstance.result.then(function (servicePriceData) {
                $scope.servicePriceData = servicePriceData;
                var sum = 0
                for(var i=0;i<servicePriceData.length;i++){
                    sum += servicePriceData[i].price
                }
        ...
        $scope.isAllPriceNotDistributed = function (){
            return $scope.offer.offerBundlingProfile && $scope.offer.offerBundlingProfile.BundlingEnabled &&  $scope.offer.offerBundlingProfile.BundlingOption === "MOBILY" && $scope.remainingServicePrice !== 0;
        }

        $scope.showOrganizations = function (organizationButton) {
            var modalInstance = $uibModal.open({
            ...
            modalInstance.result.then(function (selectedItem) {
                if(!$scope.isStcBundle()){
                    $scope.resetAllServiceAndOrganizationInfo();
                }
                $scope.organizationDictionary[selectedItem.organization.id] = selectedItem.organization.name;
                if(organizationButton){
                    ...
                    $scope.offer.tempOrganization = selectedItem.organization;
                    $scope.offer.tempOrganizationId = selectedItem.organization.id;
                }
                if($scope.isStcBundle() && !organizationButton)
                    $scope.showOnlyServices();
        ...
        $scope.isStcBundle = function (){
            if (!$scope.offer || !$scope.offer.offerBundlingProfile)
                return false;
            return $scope.offer.offerBundlingProfile.BundlingEnabled && $scope.offer.offerBundlingProfile.BundlingOption === 'MOBILY';
        }
        ...
        $scope.showOnlyServices = function () {
            var oldServices = angular.copy($scope.selectedServices);
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    ...
                    services: function ($q, CMPFService, DEFAULT_REST_QUERY_LIMIT) {
                        ...
                        var id = $scope.offer.organizationId;
                        if($scope.isStcBundle())
                            id = $scope.offer.tempOrganizationId;

                        if (id) {
                            CMPFService.getAllServicesByOrganizationId(id, false, false, 'ACTIVE', null).then(function (services) {
                                /*
            ...
            modalInstance.result.then(function (selectedItems) {
                ...
                $scope.selectedServices = oldServices;
                //
            });
        };

        $scope.isValidServices = function (){
            if (!$scope.selectedServices || ($scope.selectedServices && $scope.selectedServices.length === 0))
                return true
            if ($scope.offer && $scope.offer.offerBundlingProfile.BundlingEnabled && $scope.offer.offerBundlingProfile.BundlingOption === "MOBILY")
                return true
            var org = $scope.selectedServices[0] ? $scope.selectedServices[0].organizationId : -1;
            if (org === -1)
                return true;
            for(var i = 1; i<$scope.selectedServices.length; i++){
                if ($scope.selectedServices[i].organizationId !== org)
                    return false
            }
            return true
        }
        ...
        $scope.searchOffers = _.throttle(function (text) {
            $scope.mainOfferList = [];

            var promises = [];

            if (text && text.length > 2) {
                promises.push(CMPFService.getOffersByName(0, DEFAULT_REST_QUERY_LIMIT, false, false, false, text));
            } else {
                promises.push(CMPFService.getAllOffers());
            }

            $q.all(promises).then(function (responses) {
                _.each(responses, function (response) {
                    $scope.mainOfferList = $scope.mainOfferList.concat(response ? response.offers : []);
                })

                $scope.mainOfferList = $filter('orderBy')($scope.mainOfferList, ['name']);
            });
        }, 500);

        $scope.searchCustomerProfilingProfiles = _.throttle(function () {
            $scope.eligibilityEvalCustomerProfiles = [];

            var promise = CMPFService.getAllOrganizationsByName(0, DEFAULT_REST_QUERY_LIMIT, CMPFService.DEFAULT_CUSTOMER_PROFILES_ORGANIZATION_NAME);

            promise.then(function (response) {
                var customerProfilingOrganization = response.organizations[0];
                var customerprofilings = CMPFService.getCustomerProfilings(customerProfilingOrganization);

                $scope.eligibilityEvalCustomerProfiles = $filter('orderBy')(customerprofilings, 'Name');

                $scope.$emit('eligibilityEvalCustomerProfilesReady');
            });
        }, 500);
        ...
        $scope.perpareOfferBundlingRevshareProfile = function (offerBundlingRevshareProfile){
            return {
                name: CMPFService.OFFER_BUNDLING_REVSHARE_PROFILE,
                profileDefinitionName: CMPFService.OFFER_BUNDLING_REVSHARE_PROFILE,
                attributes: [
                    {
                        "name": "ServiceOrganization",
                        "value": offerBundlingRevshareProfile.organization
                    },
                    {
                        "name": "Service",
                        "value": offerBundlingRevshareProfile.name
                    },
                    {
                        "name": "ServiceSettlementAmount",
                        "value": offerBundlingRevshareProfile.price
                    },
                    {
                        "name": "ServiceSettlementPercentage",
                        "value": offerBundlingRevshareProfile.percentage
                    },
                    {
                        "name": "ServiceId",
                        "value": offerBundlingRevshareProfile.serviceId
                    },
                    {
                        "name": "ServiceOrganizationId",
                        "value": offerBundlingRevshareProfile.organizationId
                    }
                ]
            };
        }

        $scope.prepareOfferBundlingProfile = function (offerBundlingProfile) {
            return {
                ...
                attributes: [
                    {
                        ...
                        "value": offerBundlingProfile.BundlingEnabled
                    },
                    {
                        "name": "BundlingType",
                        "value": offerBundlingProfile.BundlingType
                    },
                    {
                        "name": "EligibilityEvalPolicy",
                        "value": offerBundlingProfile.EligibilityEvalPolicy
                    },
                    {
                        "name": "EligibilityEvalCustomerProfiles",
                        "value": offerBundlingProfile.EligibilityEvalCustomerProfiles
                    },
                    {
                        "name": "MainOfferEvalPolicy",
                        "value": offerBundlingProfile.MainOfferEvalPolicy
                    },
                    {
                        "name": "MainOfferList",
                        "value": offerBundlingProfile.MainOfferList
                    }
                ]
            };
        };
    ...
    SubscriptionManagementOperationsOffersModule.controller('SubscriptionManagementOperationsOffersCtrl', function ($rootScope, $scope, $log, $state, $uibModal, $filter, $translate, notification, NgTableParams,
        ...
        $scope.updateOfferStateByStcUser = function (offer, newState) {
            offer.rowSelected = true;
