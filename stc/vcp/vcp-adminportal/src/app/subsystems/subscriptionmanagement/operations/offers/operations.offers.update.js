(function () {
    ...
    angular.module('adminportal.subsystems.subscriptionmanagement.operations.offers.update', []);

    var SubscriptionManagementOperationsUpdateOffersModule = angular.module('adminportal.subsystems.subscriptionmanagement.operations.offers.update');

    SubscriptionManagementOperationsUpdateOffersModule.config(function ($stateProvider) {

        $stateProvider.state('subsystems.subscriptionmanagement.operations.offers.update', {
            url: "/update/:id",
            templateUrl: "subsystems/subscriptionmanagement/operations/offers/operations.offers.detail.html",
            controller: 'SubscriptionManagementOperationsOffersUpdateCtrl',
            resolve: {
                ...
                offer: function ($stateParams, $q, CMPFService, Restangular) {
                    var deferred = $q.defer();

                    CMPFService.getOffer($stateParams.id, true, true).then(function (offer) {
                        CMPFService.getOperator(offer.organizationId, true).then(function (organization) {
                            offer.organization = Restangular.stripRestangular(organization)
    ...
    SubscriptionManagementOperationsUpdateOffersModule.controller('SubscriptionManagementOperationsOffersUpdateCtrl', function ($rootScope, $scope, $log, $controller, $q, $filter, $uibModal, notification, $translate, Restangular,
                                                                                                                                UtilService, CMPFService, SessionService, DateTimeConstants, WorkflowsService, offer, shortCodesOrganization,
                                                                                                                                ContentManagementService, FileDownloadService, businessTypesOrganization, settlementTypesOrganization,
                                                                                                                                serviceCategoriesOrganization, packageList) {
        $log.debug('SubscriptionManagementOperationsOffersUpdateCtrl');
        ...
        var organizationPromise =  CMPFService.getAllOperatorsAndPartners(false, true, [CMPFService.OPERATOR_PROFILE, CMPFService.SERVICE_PROVIDER_BUSINESS_TYPE_PROFILE, CMPFService.SERVICE_PROVIDER_SETTLEMENT_TYPE_PROFILE]);

        $scope.organizationDictionary = {};
        organizationPromise.then(function(result) {
            if(result && result.organizations){
                for(var i=0; i<result.organizations.length; i++) {
                    $scope.organizationDictionary[result.organizations[i].id] = result.organizations[i].name;
                }
            }
        }, function(reason) {
            $log.debug('Organization list cant read: ', reason);
        });

    var emptyCmpfObj = { profiles: [] };
        $scope.shortCodesOrganization = shortCodesOrganization.organizations[0] ? shortCodesOrganization.organizations[0] : emptyCmpfObj;
        ...
        if (xsmOfferProfiles.length > 0) {
            ...
            if ($scope.offer.xsmOfferProfile.IsRbt) {
                // Get the coverImage by CoverImageID value.
                $scope.offer.xsmOfferProfile.coverImage = {name: undefined};
                if ($scope.offer.xsmOfferProfile.CoverImageID) {
                    var srcUrl = ContentManagementService.generateFilePath($scope.offer.xsmOfferProfile.CoverImageID);
                    FileDownloadService.downloadFileAndGetBlob(srcUrl, function (blob, fileName) {
                        $scope.offer.xsmOfferProfile.coverImage = blob;
                        if (blob) {
                            $scope.offer.xsmOfferProfile.coverImage.name = fileName;
                        }
                ...
                $scope.offer.xsmOfferProfile.cardImage = {name: undefined};
                if ($scope.offer.xsmOfferProfile.CardImageID) {
                    var srcUrl = ContentManagementService.generateFilePath($scope.offer.xsmOfferProfile.CardImageID);
                    FileDownloadService.downloadFileAndGetBlob(srcUrl, function (blob, fileName) {
                        $scope.offer.xsmOfferProfile.cardImage = blob;
                        if (blob) {
                            $scope.offer.xsmOfferProfile.cardImage.name = fileName;
                        }
                        $scope.originalOffer = angular.copy($scope.offer);
                    });
                }
                ...
                $scope.offer.xsmOfferProfile.thumbnail = {name: undefined};
                if ($scope.offer.xsmOfferProfile.ThumbnailID) {
                    var srcUrl = ContentManagementService.generateFilePath($scope.offer.xsmOfferProfile.ThumbnailID);
                    FileDownloadService.downloadFileAndGetBlob(srcUrl, function (blob, fileName) {
                        $scope.offer.xsmOfferProfile.thumbnail = blob;
                        if (blob) {
                            $scope.offer.xsmOfferProfile.thumbnail.name = fileName;
                        }
                        $scope.originalOffer = angular.copy($scope.offer);
                    });
                }
        ...
        var offerBundlingProfiles = CMPFService.getProfileAttributes($scope.offer.profiles, CMPFService.OFFER_BUNDLING_PROFILE);
        if (offerBundlingProfiles.length > 0) {
            $scope.offer.offerBundlingProfile = angular.copy(offerBundlingProfiles[0]);
            if ($scope.offer.offerBundlingProfile.BundlingEnabled) {
                if ($scope.offer.offerBundlingProfile.BundlingType === "STC"){
                    $scope.offer.offerBundlingProfile.BundlingOption = "STC"
                    var revShareProfiles = CMPFService.getProfileAttributes($scope.offer.profiles, CMPFService.OFFER_BUNDLING_REVSHARE_PROFILE);
                    if(revShareProfiles && revShareProfiles.length>0){
                        $scope.servicePriceData = []
                        var sum = 0
                        for (var i = 0; i<revShareProfiles.length; i++){
                            var data = {
                                name: revShareProfiles[i].Service,
                                organization: revShareProfiles[i].ServiceOrganization,
                                price: revShareProfiles[i].ServiceSettlementAmount,
                                percentage: revShareProfiles[i].ServiceSettlementPercentage,
                                serviceId: revShareProfiles[i].ServiceId,
                                organizationId : revShareProfiles[i].ServiceOrganizationId
                            }
                            sum += revShareProfiles[i].ServiceSettlementAmount
                            $scope.servicePriceData.push(data);
                        }
                        $scope.originalServicePriceData = angular.copy($scope.servicePriceData);
                        $scope.remainingServicePrice = $scope.offer.xsmChargingProfile.MainPrice - sum;
                    }
                }
                else{
                    $scope.offer.offerBundlingProfile.BundlingOption = "FTTH"
                }
                // MainOfferList conversions
                var copyMainOfferList = angular.copy($scope.offer.offerBundlingProfile.MainOfferList);
                if (copyMainOfferList) {
                    $scope.offer.offerBundlingProfile.MainOfferList = [];
                    _.each(copyMainOfferList.toString().split('|'), function (mainOfferId) {
                        $scope.offer.offerBundlingProfile.MainOfferList.push(Number(mainOfferId));
                    });
                }

                // EligibilityEvalCustomerProfiles conversions
                $scope.$on('eligibilityEvalCustomerProfilesReady', function (event) {
                    var copyEligibilityEvalCustomerProfiles = angular.copy($scope.offer.offerBundlingProfile.EligibilityEvalCustomerProfiles);
                    if (copyEligibilityEvalCustomerProfiles && !_.isArray(copyEligibilityEvalCustomerProfiles)) {
                        $scope.offer.offerBundlingProfile.EligibilityEvalCustomerProfiles = [];
                        _.each(copyEligibilityEvalCustomerProfiles.toString().split('|'), function (profileName) {
                            var foundEligibilityEvalCustomerProfile = _.findWhere($scope.eligibilityEvalCustomerProfiles, {'Name': profileName});
                            $scope.offer.offerBundlingProfile.EligibilityEvalCustomerProfiles.push(foundEligibilityEvalCustomerProfile);
                        });
                    }
                });
            } else {
                $scope.offer.offerBundlingProfile.BundlingType = null;
            }
        } else {
            $scope.offer.offerBundlingProfile = {BundlingEnabled: false};
        }
        ...
        $scope.isNotChanged = function () {
            return angular.equals($scope.offer, $scope.originalOffer) &&
                ...
                angular.equals($scope.originalServicePriceData, $scope.servicePriceData);
        };
        $scope.save = function (offer) {
            ...
            // XsmOfferProfile
            var coverImage, cardImage, thumbnail, updatedXsmOfferProfile;
            if (offer.xsmOfferProfile) {
                ...
                updatedXsmOfferProfile = JSON.parse(angular.toJson(offer.xsmOfferProfile));
                ...
                // Create or Update the images if there is some changes
                // CoverImageID
                coverImage = offer.xsmOfferProfile.coverImage;
                if (!coverImage || (coverImage && !coverImage.name)) {
                    updatedXsmOfferProfile.CoverImageID = '';
                } else if (coverImage instanceof File && !updatedXsmOfferProfile.CoverImageID) {
                    updatedXsmOfferProfile.CoverImageID = UtilService.generateObjectId();
                }
                delete updatedXsmOfferProfile.coverImage;
                // CardImageID
                cardImage = offer.xsmOfferProfile.cardImage;
                if (!cardImage || (cardImage && !cardImage.name)) {
                    updatedXsmOfferProfile.CardImageID = '';
                } else if (cardImage instanceof File && !updatedXsmOfferProfile.CardImageID) {
                    updatedXsmOfferProfile.CardImageID = UtilService.generateObjectId();
                }
                delete updatedXsmOfferProfile.cardImage;
                // ThumbnailID
                thumbnail = offer.xsmOfferProfile.thumbnail;
                if (!thumbnail || (thumbnail && !thumbnail.name)) {
                    updatedXsmOfferProfile.ThumbnailID = '';
                } else if (thumbnail instanceof File && !updatedXsmOfferProfile.ThumbnailID) {
                    updatedXsmOfferProfile.ThumbnailID = UtilService.generateObjectId();
                }
                delete updatedXsmOfferProfile.thumbnail;
            ...
            if (offer.offerBundlingProfile) {
                var copyProfile = angular.copy(offer.offerBundlingProfile);
                if (copyProfile.BundlingOption)
                    delete copyProfile.BundlingOption;
                var originalOfferBundlingProfile = CMPFService.findProfileByName(offerItem.profiles, CMPFService.OFFER_BUNDLING_PROFILE);
                var updatedOfferBundlingProfile = JSON.parse(angular.toJson(copyProfile));

                if (updatedOfferBundlingProfile.BundlingEnabled && offer.offerBundlingProfile.BundlingOption !== "STC") {
                    updatedOfferBundlingProfile.MainOfferList = updatedOfferBundlingProfile.MainOfferList.join('|');

                    if (updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles && updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles.length > 0) {
                        updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles = _.pluck(updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles, 'Name').join('|');
                    } else {
                        updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles = '';
                    }
                } else {
                    if (offer.offerBundlingProfile.BundlingEnabled && offer.offerBundlingProfile.BundlingOption === "STC" && !angular.equals($scope.originalServicePriceData, $scope.servicePriceData)){
                        // set state to pending needed new approvals.
                        offerItem.state = 'PENDING';
                        // remove older revshare profiles
                        offerItem.profiles = _.filter(offerItem.profiles, function (profile) {
                            return profile.name !== CMPFService.OFFER_BUNDLING_REVSHARE_PROFILE;
                        });
                    ...
                    updatedOfferBundlingProfile.BundlingType = "STC";
                    updatedOfferBundlingProfile.EligibilityEvalPolicy = '';
                    updatedOfferBundlingProfile.EligibilityEvalCustomerProfiles = '';
                    updatedOfferBundlingProfile.MainOfferEvalPolicy = '';
                    updatedOfferBundlingProfile.MainOfferList = '';
                }
                var offerBundlingProfileArray = CMPFService.prepareProfile(updatedOfferBundlingProfile, originalOfferBundlingProfile);
                // ---
                if (originalOfferBundlingProfile) {
                    originalOfferBundlingProfile.attributes = offerBundlingProfileArray;
                } else {
                    var offerBundlingProfile = {
                        name: CMPFService.OFFER_BUNDLING_PROFILE,
                        profileDefinitionName: CMPFService.OFFER_BUNDLING_PROFILE,
                        attributes: offerBundlingProfileArray
                    };
            ...
            WorkflowsService.updateOffer(offerItemPayload).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    if (updatedXsmOfferProfile) {
                        if (coverImage && coverImage.name && (coverImage instanceof File)) {
                            promises.push(ContentManagementService.uploadFile(coverImage, coverImage.name, updatedXsmOfferProfile.CoverImageID));
                        }

                        if (cardImage && cardImage.name && (cardImage instanceof File)) {
                            promises.push(ContentManagementService.uploadFile(cardImage, cardImage.name, updatedXsmOfferProfile.CardImageID));
                        }

                        if (thumbnail && thumbnail.name && (thumbnail instanceof File)) {
                            promises.push(ContentManagementService.uploadFile(thumbnail, thumbnail.name, updatedXsmOfferProfile.ThumbnailID));
                        }
                    }
            ...
            /*
            CMPFService.updateOffer(offerItem).then(function (response) {
                ...
                if (response && response.errorCode) {
                    ...
                    $scope.go('subsystems.subscriptionmanagement.operations.offers.list');
                }
            }, function (response) {
