(function () {
        ...
        $stateProvider.state('subsystems.contentmanagement.operations.rbt.contentmetadatas.tones', {
            ...
            resolve: {
                tone: function ($stateParams, $q, ContentManagementService, CMPFService) {
                    ...
                    ContentManagementService.getTone($stateParams.id, ['CATEGORY', 'SUBCATEGORY']).then(function (_tone) {
    ...
    ContentManagementOperationsContentMetadatasRBTTonesModule.controller('ContentManagementOperationsContentMetadatasRBTTonesCommonCtrl', function ($scope, $log, $q, $state, $controller, $uibModal, $filter, $translate, UtilService, SessionService, ContentManagementService, CMPFService,
                                                                                                                                                    CMS_RBT_STATUS_TYPES, CMS_GENDERS, CMS_CODECS, CMS_ACCESS_CHANNELS, DURATION_UNITS, CMS_RBT_REV_SHARE_POLICIES_TONE,
        ...
        $scope.CMS_ACCESS_CHANNELS = _.flatten(_.map(CMS_ACCESS_CHANNELS, 'value'));
        ...
        $scope.openCategorySelection = function (organizationId) {
            ...
            $scope.showAllowedCategories(organizationId, $scope.contentMetadata.subcategories, setCategorySelection, config)
        };
        ...
        */
        ...
        $scope.$watch('contentMetadata.isBlacklisted', function (newValue, oldValue) {
            $log.debug('contentMetadata: ', $scope.contentMetadata, 'contentMetadata.isBlacklisted: ', newValue, oldValue);
            if(newValue){
                $scope.contentMetadata.promoted = false;
            }
        });

        $scope.$watch('contentMetadata.promoted', function (newValue, oldValue) {
            $log.debug('contentMetadata: ', $scope.contentMetadata, 'contentMetadata.promoted: ', newValue, oldValue);
            if(newValue){
                $scope.contentMetadata.isBlacklisted = false;
            }
        });
        ...
        // Call the copyright file controller so its scope could be used with this controller.
        ...
        $scope.exportOptions = {
            columns: [
                ...
                },
                {
                    ...
                    headerKey: 'Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Blacklisted',
                    filter: {name: 'YesNoFilter'}
                },
                {
        ...
        $scope.remove = function (contentMetadata) {
            ...
            modalInstance.result.then(function () {
                ...
                var toneItem = {
                    ...
                    // "toneFileUrl": contentMetadata.toneFileUrl,
                ...
                ContentManagementService.deleteTone(toneItem).then(function (response) {
                    $log.debug('Removing tone: ', toneItem, ', response: ', response);
                    ...
                    $log.error('Cannot call the tone delete service. Error: ', response);

                    if (response && response.data && response.data.message) {
                        WorkflowsService.showApiError(response);
                    } else {
                        var message = $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Tones.Messages.DeleteFlowError');
    ...
    ContentManagementOperationsContentMetadatasRBTTonesModule.controller('ContentManagementOperationsContentMetadatasRBTTonesUpdateCtrl', function ($rootScope, $scope, $state, $log, $controller, $q, $filter, $translate, notification, UtilService, Restangular,
        ...
        $scope.sendToWorkflow = function (contentMetadata) {
            ...
            var toneItem = {
                ...
                "offers": original.offers, // offers will be overwritten below if chargingDetails present
                ...
                "organizationName": contentMetadata.organization.name,
                "status": contentMetadata.status,
                "rejectReason": contentMetadata.rejectReason,
                "subscriptionEnabled": contentMetadata.subscriptionEnabled,
                "promoted": contentMetadata.promoted,
                "isBlacklisted": contentMetadata.isBlacklisted,
                "launchDate": ($scope.dateHolder.startDate ? $filter('date')($scope.dateHolder.startDate, 'yyyy-MM-dd') + 'T00:00:00' : ''),
            ...
            // Charging Details
            if (contentMetadata.chargingDetails) {
                // Force 30 days as per STC requirement
                contentMetadata.chargingDetails.chargingPeriod.duration = 30;
                contentMetadata.chargingDetails.chargingPeriod.unit = $scope.DURATION_UNITS[0].key;
            ...
            if (cardReplaced) {
                ...
                // If this fails, no metadata, no other uploads.
            }
                        ...
                        .then(function () {
                            notification.flash({
                                ...
                                text: $translate.instant(
                                    'Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Tones.Messages.UpdateFlowStartedSuccessful'
                    ...
                    return $q.all(cleanup).finally(function () {
                        ...
                        if (res && res.data && res.data.message) {
                            ...
                            notification({
                                type: 'warning',
                                text: $translate.instant(
                                    'Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Tones.Messages.UpdateFlowError'
                                )
                            });
                        }
                    });
