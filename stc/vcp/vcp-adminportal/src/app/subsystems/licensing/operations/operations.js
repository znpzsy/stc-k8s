(function () {
    ...
    LicensingOperationsModule.controller('LicensingUpdateOperationsCtrl', function ($scope, $state, $log, $q, $stateParams, $uibModal, $filter, $translate, notification,
        ...
        $scope.licenseSamplingType = license.productLicenses.entry[0].value.type;
        $scope.licenseUoM = license.uom;
        ...
        $scope.getLabels = function() {
            ...
            return {
                HardLimit: $translate.instant('Subsystems.Licensing.Operations.HardLimit') + ' (' + $translate.instant(samplingUnit) + ')',
                SoftLimit: $translate.instant('Subsystems.Licensing.Operations.SoftLimit') + ' (' + $translate.instant(samplingUnit) + ')',
                UserLimit: $translate.instant('Subsystems.Licensing.Operations.UserLimit') + ' (' + $translate.instant(samplingUnit) + ')',
            };
        };
        ...
        // For dedicated licenses of type tpp : TPM to TPS conversion for UserLimit - Service expects this param as (tpm), Screens display this as (tps)
        // For licenses of type bucket or session, this conversion will not be made.
        if (licenseType === 'dedicated' && $scope.licenseSamplingType === 'tpp') {

            var userLimit = license.productLicenses.entry[0].value.userLimit / license.limitPeriodInSeconds;
            $scope.license.productLicenses.entry[0].value.userLimit = $scope.roundUp(userLimit, 0);
        }
            ...
            modalInstance.result.then(function (productLimits) {
                ...
                for (var i = 0; i < productLimits.entry.length; i++) {
                    ...
                    var entry = {
                        ...
                        userLimit: $scope.roundUp(product.value.userLimit / 60, 0),
        ...
        $scope.save = function () {
            ...
            LicenseManagerService.setLicenseThresholds($scope.licenseName, licenseThresholds).then(function (response) {
                ...
                if ($scope.isIndividual()) {
                    $scope.productLimits = [
                        {
                            // TODO: Ask about product licenses entry[0] tresholds, is there a reason to set them to 0?
                            name: $scope.license.productLicenses.entry[0].value.name,
                            hardLimitMajorThreshold: 0, // licenseThresholds.hardLimitMajorThreshold,
                            hardLimitMinorThreshold: 0, // licenseThresholds.hardLimitMinorThreshold,
                            softLimitMajorThreshold: 0, // licenseThresholds.softLimitMajorThreshold,
                            softLimitMinorThreshold: 0, // licenseThresholds.softLimitMinorThreshold,
                            userLimit: ($scope.licenseSamplingType === 'tpp') ? parseFloat($scope.license.productLicenses.entry[0].value.userLimit) * $scope.license.limitPeriodInSeconds : $scope.license.productLicenses.entry[0].value.userLimit,
