(function () {
    ...
    angular.module('adminportal.products.otp.troubleshooting', []);

    var OTPTroubleshootingModule = angular.module('adminportal.products.otp.troubleshooting');

    OTPTroubleshootingModule.config(function ($stateProvider) {

        $stateProvider.state('products.otp.troubleshooting', {
            abstract: true,
            url: "/troubleshooting",
            templateUrl: "products/otp/troubleshooting/troubleshooting.html",
            data: {
                permissions: [
                    'READ_ALL_TROUBLESHOOTING'
                ]
            }
        }).state('products.otp.troubleshooting.sms', {
            url: "/sms",
            views: {
                'filter-form': {
                    templateUrl: 'products/otp/troubleshooting/troubleshooting.sms.filter.html',
                },
                'table': {
                    templateUrl: 'products/otp/troubleshooting/troubleshooting.sms.html',
                    controller: 'OTPTroubleshootingSMSCtrl',
                    resolve: {
    ...
    OTPTroubleshootingModule.controller('OTPTroubleshootingCommonCtrl', function ($scope, $log, $q, $timeout, $filter, $controller, notification, $translate, $uibModal, NgTableParams,
                                                                                  UtilService, DateTimeConstants, GeneralESService, TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION  ) {
        $log.debug('OTPTroubleshootingCommonCtrl');
        ...
        $scope.origAgentType = {id: 1};
        $scope.sdpService = {id: 118};
        ...
        var exportAllData = function (fileNamePrefix, exporter) {
            ...
            GeneralESService.findSmscPermanentEdrs(filter, additionalFilterFields).then(function (response) {
        ...
        $scope.prepareFilter = function (dateFilter, tableParams) {
            ...
            result.additionalFilterFields = {
                // Calling Party
                origAgentType: $scope.origAgentType.id,
    ...
    OTPTroubleshootingModule.controller('OTPTroubleshootingSMSCtrl', function ($scope, $log, $q, $filter, $controller, notification, $translate, $uibModal, Restangular, NgTableParams,
                                                                               GeneralESService, DateTimeConstants, MSGGW_SMSC_AGENT_TYPES, services) {
        $log.debug('OTPTroubleshootingSMSCtrl');
        ...
        $controller('OTPTroubleshootingCommonCtrl', {
            $scope: $scope
        });
        ...
        $scope.showDetails = function (edrRecord) {
            $scope.showDetailsCommon(edrRecord, 'products/otp/troubleshooting/troubleshooting.sms.edr.details.html');
        };
        ...
        $scope.showMessageContent = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/otp/troubleshooting/troubleshooting.sms.edr.messagecontent.html',
                ...
                resolve: {
                    ...
                    entireMsgContent: function ($q, $log, notification, $translate, GeneralESService) {
                        ...
                        if (edrRecord.partTotal === 1) {
                            ...
                            promise = GeneralESService.findSmscPermanentMessageParts(edrRecord.origAddress, edrRecord.destAddress, edrRecord.partRef, edrRecord.date);
        ...
        $scope.showMessageDeliveryReport = function (edrRecord) {
            $scope.showMessageDeliveryReportCommon(edrRecord, 'products/otp/troubleshooting/troubleshooting.sms.edr.messagedeliveryreport.html', GeneralESService.findMessagingGwSMSDeliveryReports);
        };
    });
