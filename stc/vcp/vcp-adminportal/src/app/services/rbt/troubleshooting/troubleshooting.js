(function () {
    ...
    angular.module('adminportal.services.rbt.troubleshooting', []);

    var RBTTroubleshootingModule = angular.module('adminportal.services.rbt.troubleshooting');

    RBTTroubleshootingModule.config(function ($stateProvider) {

        $stateProvider.state('services.rbt.troubleshooting', {
            url: "/troubleshooting",
            templateUrl: "services/rbt/troubleshooting/troubleshooting.html",
            controller: 'RBTTroubleshootingCtrl',
            permissions: [
                'RBT_SERVICE_TROUBLESHOOTING_READ'
            ]
        });

    });

    RBTTroubleshootingModule.controller('RBTTroubleshootingCtrl', function ($scope, $log, $controller, $timeout, $filter, DateTimeConstants) {
        $log.debug('RBTTroubleshootingCtrl');
        ...
        $controller('RBTTroubleshootingTableCtrl', {$scope: $scope});
        $controller('RBTTroubleshootingHistoryCtrl', {$scope: $scope});
    });

    RBTTroubleshootingModule.controller('RBTTroubleshootingTableCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams,
                                                                                 UtilService, GeneralESService, DateTimeConstants) {
        $log.debug('RBTTroubleshootingTableCtrl');

        $scope.exportFileName = 'RingBackToneServiceTroubleshootingRecords';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.askService) {
                    GeneralESService.findRBTHistory(filter, additionalFilterFields).then(function (response) {
    ...
    RBTTroubleshootingModule.controller('RBTTroubleshootingHistoryCtrl', function ($scope, $log, $filter, notification, $translate, $uibModal, NgTableParams,
                                                                                   GeneralESService, DateTimeConstants) {
        $log.debug('RBTTroubleshootingHistoryCtrl');
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    rbtEdrs: function (GeneralESService) {
                        return GeneralESService.findRBTDetailedHistory(edrRecord.sessionid);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
