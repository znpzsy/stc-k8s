(function () {
    ...
    angular.module('ccportal.subscriber-info.activityhistory.charginghistory', []);

    var ActivityHistoryChargingHistoryModule = angular.module('ccportal.subscriber-info.activityhistory.charginghistory');

    ActivityHistoryChargingHistoryModule.config(function ($stateProvider) {

        $stateProvider.state('subscriber-info.activityhistory.charginghistory', {
            url: "/charging-history",
            templateUrl: "subscriber-info/activity-history/charginghistory/troubleshooting.html",
            controller: 'ActivityHistoryChargingHistoryCtrl',
            data: {
                permissions: [
                    'CC__CHARGING_READ'
                ]
    ...
    ActivityHistoryChargingHistoryModule.controller('ActivityHistoryChargingHistoryCtrl', function ($scope, $log, $controller, $timeout, $filter, $uibModal, $translate, Restangular, UtilService, DateTimeConstants,
                                                                                                    TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION, GeneralESService, ReportingExportService, organizations,
                                                                                                    services, offers) {
        $log.debug('ActivityHistoryChargingHistoryCtrl');
        ...
        $controller('ActivityHistoryChargingHistoryTableCtrl', {$scope: $scope});
    });

    ActivityHistoryChargingHistoryModule.controller('ActivityHistoryChargingHistoryTableCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams,
                                                                                                         AuthorizationService, UtilService, GeneralESService, DateTimeConstants,
                                                                                                         TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION) {
        $log.debug('ActivityHistoryChargingHistoryTableCtrl');
        ...
        $scope.showDetails = function (edrRecord) {
            ...
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'subscriber-info/activity-history/charginghistory/troubleshooting.edr.details.html',
        ...
        $scope.refund = function (selectedEdrRecord) {
            ...
            var modalInstance = $uibModal.open({
                templateUrl: 'subscriber-info/activity-history/charginghistory/troubleshooting.edr.refundconfirmation.modal.html',
                size: 'lg',
                controller: 'ActivityHistoryChargingHistoryRefundModalCtrl',
                resolve: {
    ...
    ActivityHistoryChargingHistoryModule.controller('ActivityHistoryChargingHistoryRefundModalCtrl', function ($scope, $log, notification, $translate, $filter, $uibModal, $uibModalInstance, NgTableParams, NgTableService, Restangular,
                                                                                                               AuthorizationService, DateTimeConstants, UtilService, ChargingGwService, selectedEdrRecord, chargingRefunds) {
        $log.debug('ActivityHistoryChargingHistoryRefundModalCtrl');
        ...
        $scope.refund = function (selectedEdrRecord, chargingRefund) {
            var chargingRefundItem = {
                ...
                "clientCorrelator": 'csp_ccportal_' + UtilService.getCurrentNanoTime()
            };

            if (AuthorizationService.canCcChargingRefund() && selectedEdrRecord.amount && Number(selectedEdrRecord.amount) > 0 && chargingRefund.amount > 0) {
