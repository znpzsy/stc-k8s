(function () {
    ...
    angular.module('adminportal.subsystems.contentmanagement.operations.rbt.contentmetadatas.albums', []);

    var ContentManagementOperationsContentMetadatasRBTAlbumsModule = angular.module('adminportal.subsystems.contentmanagement.operations.rbt.contentmetadatas.albums');

    ContentManagementOperationsContentMetadatasRBTAlbumsModule.config(function ($stateProvider) {

        $stateProvider.state('subsystems.contentmanagement.operations.rbt.contentmetadatas.albums', {
            abstract: true,
            url: '/albums',
            template: '<div ui-view></div>',
            data: {
                exportFileName: 'ContentsAlbumsRBT',
                permissions: [
                    'RBT__OPERATIONS_ALBUM_READ'
                ]
            }
        }).state('subsystems.contentmanagement.operations.rbt.contentmetadatas.albums.list', {
            url: "",
            templateUrl: "subsystems/contentmanagement/operations/rbt/operations.contentmetadatas.albums.html",
            controller: 'ContentManagementOperationsContentMetadatasRBTAlbumsCtrl'
        }).state('subsystems.contentmanagement.operations.rbt.contentmetadatas.albums.new', {
            url: "/new",
            templateUrl: "subsystems/contentmanagement/operations/rbt/operations.contentmetadatas.albums.details.html",
            controller: 'ContentManagementOperationsContentMetadatasRBTAlbumsNewCtrl'
        }).state('subsystems.contentmanagement.operations.rbt.contentmetadatas.albums.update', {
            url: "/update/:id",
            templateUrl: "subsystems/contentmanagement/operations/rbt/operations.contentmetadatas.albums.details.html",
            controller: 'ContentManagementOperationsContentMetadatasRBTAlbumsUpdateCtrl',
            resolve: {
                album: function ($stateParams, $q, ContentManagementService, CMPFService) {
                    var deferred = $q.defer();

                    ContentManagementService.getAlbum($stateParams.id).then(function (_album) {
                        CMPFService.getOperator(_album.album.organizationId, true).then(function (_organization) {
                            _album.album.organization = _organization;

                            deferred.resolve(_album);
                        });
                    });
    ...
    ContentManagementOperationsContentMetadatasRBTAlbumsModule.controller('ContentManagementOperationsContentMetadatasRBTAlbumsCommonCtrl', function ($scope, $log, $q, $state, $controller, $uibModal, $filter, UtilService, SessionService, ContentManagementService, CMPFService,
        ...
        $log.debug('ContentManagementOperationsContentMetadatasRBTAlbumsCommonCtrl');
        ...
        $scope.openOrganizations = function (album) {
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    organizationParameter: function () {
                        return angular.copy(album.organization);
                    },
                    itemName: function () {
                        return album.name;
                    },
                    ...
                    organizationsModalTitleKey: function () {
                        return 'Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.OrganizationsModalTitle';
                    }
                }
            });

            modalInstance.result.then(function (selectedItem) {
                album.organization = selectedItem.organization;
                album.organizationId = selectedItem.organization.id;
                album.defaultToneId = null;
            }, function () {
            });
        };
        ...
        $scope.cancel = function () {
            $state.go('subsystems.contentmanagement.operations.rbt.contentmetadatas.albums.list');
        };
    });

    ContentManagementOperationsContentMetadatasRBTAlbumsModule.controller('ContentManagementOperationsContentMetadatasRBTAlbumsCtrl', function ($rootScope, $scope, $log, $controller, $state, $timeout, $uibModal, $filter, $translate, notification, NgTableParams, NgTableService,
        ...
        $log.debug('ContentManagementOperationsContentMetadatasRBTAlbumsCtrl');
        ...
        $scope.contentMetadataList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.status !== 'WAITING') {
                    ContentManagementService.getAlbums(filter.page, filter.limit, filter.sortFieldName, filter.sortOrder, filter.statuses, filter.filterText).then(function (response) {
        ...
        var prepareAlbumItem = function (contentMetadata, isForceAdmin) {
            var albumItem = {
                "from": {
        ...
        $scope.manageRevenueSettings = function (contextKey, entity, titleKey) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($scope, $filter, $uibModalInstance, tones, Restangular, NgTableParams, NgTableService) {
                    ...
                    $scope.save = function (album, tones) {
                        var albumItem = prepareAlbumItem(album, true);

                        var offers = albumItem.offers;
                        ...
                        WorkflowsService.updateAlbumRBT(albumItem).then(function (response) {
                            if (response && response.code === 2001) {
                                ...
                                notification({
                                    type: 'success',
                                    text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.UpdateFlowStartedSuccessful' + ($rootScope.isAdminUser ? 'ForAdmin' : ''))
                                });
                            ...
                            if (response && response.data && response.data.message) {
                                ...
                                notification({
                                    type: 'warning',
                                    text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.UpdateFlowError')
                                });
                            }
                        });
                    };
        ...
        $scope.remove = function (contentMetadata) {
            ...
            modalInstance.result.then(function () {
                ...
                var albumItem = prepareAlbumItem(contentMetadata, false);
                ...
                WorkflowsService.deleteAlbumRBT(albumItem).then(function (response) {
                    if (response && response.code === 2001) {
                        ...
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.DeleteFlowStartedSuccessful' + ($rootScope.isAdminUser ? 'ForAdmin' : ''))
                        });
                    ...
                    if (response && response.data && response.data.message) {
                        ...
                        notification({
                            type: 'warning',
                            text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.DeleteFlowError')
                        });
                    }
                });
            }, function () {
    ...
    ContentManagementOperationsContentMetadatasRBTAlbumsModule.controller('ContentManagementOperationsContentMetadatasRBTAlbumsNewCtrl', function ($rootScope, $scope, $log, $state, $controller, $q, $filter, $translate, notification, UtilService, ContentManagementService,
                                                                                                                                                   CMPFService, WorkflowsService, SessionService, DateTimeConstants) {
        $log.debug('ContentManagementOperationsContentMetadatasRBTAlbumsNewCtrl');

        $controller('ContentManagementOperationsContentMetadatasRBTAlbumsCommonCtrl', {
            $scope: $scope
        });
            ...
            WorkflowsService.createAlbumRBT(albumItem).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    $q.all(promises).then(function () {
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.CreateFlowStartedSuccessful' + ($rootScope.isAdminUser ? 'ForAdmin' : ''))
                        });
                ...
                if (response && response.data && response.data.message) {
                    ...
                    notification({
                        type: 'warning',
                        text: $translate.instant('Subsystems.ContentManagement.Operations.RBT.ContentMetadatas.Albums.Messages.CreateFlowError')
                    });
                }
            });
        };
    ...
    ContentManagementOperationsContentMetadatasRBTAlbumsModule.controller('ContentManagementOperationsContentMetadatasRBTAlbumsUpdateCtrl', function ($rootScope, $scope, $state, $log, $controller, $q, $filter, $translate, notification, Restangular, UtilService, CMPFService,
                                                                                                                                                      ContentManagementService, WorkflowsService, FileDownloadService, SessionService, DateTimeConstants,
                                                                                                                                                      album) {
        $log.debug('ContentManagementOperationsContentMetadatasRBTAlbumsUpdateCtrl');
        ...
        if ($scope.contentMetadata.defaultToneId && !album.defaultTone) {
            $scope.toneList.push({
