(function () {
    ...
    ProvisioningUsersGroupsOperationsModule.config(function ($stateProvider) {
        ...
        $stateProvider.state('subsystems.provisioning.operations.users.groups', {
            ...
            data: {
                permissions: [
                    'CMPF__OPERATIONS_USERGROUP_READ'
                ]
            }
    ...
    // Groups controllers
    ProvisioningUsersGroupsOperationsModule.controller('ProvisioningOperationsUserGroupsCommonCtrl', function ($scope, $log, $filter, $uibModal, notification, $translate, UtilService, CMPFService) {
        ...
        var addToList = function (list, isResource, priviledge, item) {
            var resourceId = priviledge.resourceId, operationId = priviledge.operationId;
            if (isResource) {
                operationId = 0;
            }
        ...
        var preparePermissionOrConstraint = function (permissionTag, groupName, isResource, priviledge) {
            var permission = {
                name: groupName + ':' + permissionTag + ':' + priviledge.resourceName,
                operationId: priviledge.operationId,
                resourceId: priviledge.resourceId,
                resourceName: priviledge.resourceName
            };

            if (isResource) {
                permission.operationId = 0;
            } else {
                permission.name = permission.name + ':' + priviledge.operationId;
                permission.operationName = priviledge.operationName;
            }
        ...
        $scope.openPermissions = function () {
            ...
            modalInstance.result.then(function (privileges) {
                ...
                $scope.permissions = $filter('orderBy')($scope.permissions, 'name');
                $scope.constraints = $filter('orderBy')($scope.constraints, 'name');
            }, function () {
            });
        };
        ...
        $scope.addPermissions = function (userGroup) {
            if (angular.equals($scope.permissions, $scope.permissionsOriginal)) {
                return userGroup;
            }
        ...
        $scope.addConstraints = function (userGroup) {
            if (angular.equals($scope.constraints, $scope.constraintsOriginal)) {
                return userGroup;
            }
        ...
        $scope.newEmailAddress = function (emailAddress) {
            var isValid = UtilService.Validators.EmailAddress.test(emailAddress);

            $scope.form.EmailAddressList.$setDirty();
            $scope.form.EmailAddressList.$setValidity('pattern', isValid);

            return isValid ? {value: emailAddress} : undefined;
        };

        $scope.prepareNewGroupProfile = function (groupProfile) {
            return {
                "name": CMPFService.GROUP_PROFILE_NAME,
                "profileDefinitionName": CMPFService.GROUP_PROFILE_NAME,
                "attributes": [
                    {
                        "name": "isRoleBased",
                        "value": groupProfile.isRoleBased
                    },
                    {
                        "name": "EmailAddressList",
                        "listValues": groupProfile.EmailAddressList
                    }
                ]
            };
        };
    ...
    ProvisioningUsersGroupsOperationsModule.controller('ProvisioningOperationsNewUserGroupCtrl', function ($scope, $state, $log, $controller, $uibModal, notification, $translate, Restangular,
        ...
        $scope.save = function (userGroup) {
            ...
            var newGroupProfile = $scope.prepareNewGroupProfile(userGroup.groupProfile);
            userGroupItem.profiles.push(newGroupProfile);
    ...
    ProvisioningUsersGroupsOperationsModule.controller('ProvisioningOperationsUserGroupUpdateCtrl', function ($scope, $state, $log, $q, $stateParams, $controller, $filter, notification, $translate, $uibModal, CMPFService, Restangular,
                                                                                                              STATUS_TYPES, userGroup, userGroupPermissions, userGroupConstraints, userGroupMembers) {
        ...
        // GroupProfile
        var groupProfiles = CMPFService.getProfileAttributes($scope.userGroup.profiles, CMPFService.GROUP_PROFILE_NAME);
        if (groupProfiles.length > 0) {
            $scope.userGroup.groupProfile = angular.copy(groupProfiles[0]);

            $scope.userGroup.groupProfile.EmailAddressList
        } else {
            $scope.userGroup.groupProfile = {
                isRoleBased: false,
                EmailAddressList: []
            };
        }
        ...
        $scope.permissionsOriginal = angular.copy($scope.permissions);
        $scope.constraintsOriginal = angular.copy($scope.constraints);
        ...
        $scope.isNotChanged = function () {
            return angular.equals($scope.userGroup, $scope.userGroupOriginal) &&
                angular.equals($scope.permissions, $scope.permissionsOriginal) &&
                angular.equals($scope.constraints, $scope.constraintsOriginal) &&
        ...
        $scope.save = function (userGroup) {
            var userGroupItem = {
                ...
                name: userGroup.name,
                state: userGroup.state,
                profiles: ($scope.userGroupOriginal.profiles === undefined ? [] : $scope.userGroupOriginal.profiles)
            };


            // GroupProfile
            if (userGroup.groupProfile) {
                var originalGroupProfile = CMPFService.findProfileByName(userGroupItem.profiles, CMPFService.GROUP_PROFILE_NAME);
                var updatedGroupProfile = JSON.parse(angular.toJson(userGroup.groupProfile));
                var groupProfileArray = CMPFService.prepareProfile(updatedGroupProfile, originalGroupProfile);
                // ---
                if (originalGroupProfile) {
                    originalGroupProfile.attributes = groupProfileArray;
                } else {
                    var groupProfile = {
                        name: CMPFService.GROUP_PROFILE_NAME,
                        profileDefinitionName: CMPFService.GROUP_PROFILE_NAME,
                        attributes: groupProfileArray
                    };

                    userGroupItem.profiles.push(groupProfile);
                }
            }
    ...
    var PrivilegesModalInstanceCtrl = function ($scope, $log, $uibModalInstance, $filter, NgTableParams, Restangular, resources,
                                                userGroup, permissionsParameter, constraintsParameter) {
    ...
    var UserAccountsModalInstanceCtrl = function ($scope, $uibModalInstance, $log, $filter, NgTableParams, NgTableService, Restangular,
                                                  userAccountsParameter, userGroup, userAccounts) {
