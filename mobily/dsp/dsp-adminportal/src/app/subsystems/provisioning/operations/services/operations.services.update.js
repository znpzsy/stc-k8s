(function () {
    ...
    angular.module('adminportal.subsystems.provisioning.operations.services.update', []);

    var ProvisioningOperationsUpdateServicesModule = angular.module('adminportal.subsystems.provisioning.operations.services.update');

    ProvisioningOperationsUpdateServicesModule.config(function ($stateProvider) {

        $stateProvider.state('subsystems.provisioning.operations.services.update', {
            url: "/update/:id",
            ...
            resolve: {
                ...
                service: function ($stateParams, $q, CMPFService, Restangular) {
                    var deferred = $q.defer();

                    CMPFService.getService($stateParams.id).then(function (service) {
                        CMPFService.getOperator(service.organizationId, true).then(function (organization) {
                            service.organization = Restangular.stripRestangular(organization);
    ...
    ProvisioningOperationsUpdateServicesModule.controller('ProvisioningOperationsUpdateServiceCtrl', function ($rootScope, $scope, $log, $controller, $q, $filter, $uibModal, notification, $translate, Restangular, UtilService,
        ...
        if ($scope.service.serviceProfile && $scope.service.serviceProfile.Type && $scope.service.serviceProfile.Type.startsWith('STANDARD_')) {
            ...
            if ($scope.service.templateAttributes && ($scope.service.serviceProfile.Template === 'ON_DEMAND' || $scope.service.serviceProfile.Template === 'OTHER')) {
                ...
                if ($scope.service.templateAttributes.ServiceProposalFileID) {
