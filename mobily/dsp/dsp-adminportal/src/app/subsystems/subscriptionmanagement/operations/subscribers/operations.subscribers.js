(function () {
    ...
    SubscriptionManagementOperationsSubscribersModule.controller('SubscriptionManagementOperationsSubscribersCtrl', function ($scope, $log, $q, $controller, $timeout, $translate, notification, $uibModal, NgTableParams, UtilService,
        ...
        $scope.filterTableBySAN = _.throttle(function (sanFilterText) {
            $scope.subscriberList.tableParams.settings().$scope.sanFilterText = sanFilterText;
            $scope.subscriberList.tableParams.page(1);
            $scope.subscriberList.tableParams.reload();
        }, 2000);
        $scope.filterTableBySANChange = function (sanFilterText) {
            if (!sanFilterText) {
                $scope.filterTableBySAN(null);
            }
        };
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    ...
                    contentSubscriptions: function (SSMSubscribersService) {
                        return SSMSubscribersService.getContentSubscriptionsByMsisdn(subscriber.msisdn);
                    }
                }
            });
    ...
    SubscriptionManagementOperationsSubscribersModule.controller('SubscriptionManagementOperationsSubscribersContentSubscriptionsModalCtrl', function ($scope, $uibModalInstance, $log, $timeout, $filter, NgTableParams, NgTableService, Restangular, SSMSubscribersService,
        ...
        $scope.contentSubscriptions = Restangular.stripRestangular(contentSubscriptions);
        if ($scope.contentSubscriptions && !$scope.contentSubscriptions.contentSubscriptions) {
            $scope.contentSubscriptions.contentSubscriptions = [];
        }
        ...
        $scope.exportOptions = {
            columns: [
                ...
                {
                    fieldName: 'chargingPeriod.lastRenewalDate',
                    headerKey: 'Subsystems.SubscriptionManagement.Operations.Subscribers.TableColumns.LastSubscriptionDate',
                    filter: {name: 'date', params: ['yyyy-MM-dd HH:mm:ss']}
                }
            ]
        ...
        $scope.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                var filterText = params.settings().$scope.filterText;
                var filterColumns = params.settings().$scope.filterColumns;
                var filteredListData = NgTableService.filterList(filterText, filterColumns, $scope.contentSubscriptions.contentSubscriptions);
                var orderedData = params.sorting() ? $filter('orderBy')(filteredListData, params.orderBy()) : $scope.contentSubscriptions.contentSubscriptions;
                params.total(orderedData.length); // set total for recalc pagination
                if ((params.total() > 0) && (params.total() === (params.count() * (params.page() - 1)))) {
                    params.page(params.page() - 1);
                }
