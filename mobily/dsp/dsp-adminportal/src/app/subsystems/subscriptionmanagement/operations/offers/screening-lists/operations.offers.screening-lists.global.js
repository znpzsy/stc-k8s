(function () {
    ...
    SubscriptionManagementOperationsOffersGlobalScreeningListsGlobalModule.config(function ($stateProvider) {
        ...
        $stateProvider.state('subsystems.subscriptionmanagement.operations.offers.screening-lists.global', {
            ...
            resolve: {
                msisdnList: function (offer, ScreeningManagerV2Service) {
                    return ScreeningManagerV2Service.getScreeningListsByScopeAndService(ScreeningManagerV2Service.serviceNames.SSM, offer.name, ScreeningManagerV2Service.scopes.MSISDN_SCOPE_KEY);
                },
                sanList: function (offer, ScreeningManagerV2Service) {
                    return ScreeningManagerV2Service.getScreeningListsByScopeAndService(ScreeningManagerV2Service.serviceNames.SSM, offer.name, ScreeningManagerV2Service.scopes.SAN_SCOPE_KEY);
                }
            }
        });
    ...
    SubscriptionManagementOperationsOffersGlobalScreeningListsGlobalModule.controller('SubscriptionManagementOperationsOffersScreeningListsGlobalListsCtrl', function ($scope, $log, $controller, $filter, $uibModal, $translate, notification, UtilService, NgTableParams, NgTableService,
                                                                                                                                                                       AdmPortalMainPromiseTracker, Restangular, ScreeningManagerV2Service, SCREENING_MANAGER_RULES, OFFER_SCREENING_IDENTIFIERS, offer,
                                                                                                                                                                       msisdnList, sanList) {
        ...
        if (msisdnList && msisdnList.screeningScope) {
            _.each(msisdnList.screeningScope.blackList, function (screenableEntry) {
                screenableEntry['serviceKey'] = ScreeningManagerV2Service.serviceNames.SSM;
                screenableEntry['identifier'] = OFFER_SCREENING_IDENTIFIERS[0];
                screenableEntry['scopeKey'] = ScreeningManagerV2Service.scopes.MSISDN_SCOPE_KEY;
        ...
        if (sanList && sanList.screeningScope) {
            _.each(sanList.screeningScope.blackList, function (screenableEntry) {
                ...
                screenableEntry['scopeKey'] = ScreeningManagerV2Service.scopes.SAN_SCOPE_KEY;
        ...
        var deleteListItem = function ($listObj, listKey, item) {
            ...
            modalInstance.result.then(function () {
                ...
                ScreeningManagerV2Service.deleteListItem(item.serviceKey, $scope.offer.name, item.scopeKey, listKey, item.screenableEntryId).then(function (response) {
        ...
        var addNewListItem = function (currentTable, listKey) {
            ...
            modalInstance.result.then(function (listItem) {
                ...
                if (listItem.identifier.value === 'MSISDN') {
                    scopeKey = ScreeningManagerV2Service.scopes.MSISDN_SCOPE_KEY;
                } else if (listItem.identifier.value === 'SAN') {
                    scopeKey = ScreeningManagerV2Service.scopes.SAN_SCOPE_KEY;
                }

                ScreeningManagerV2Service.addNewListItem(ScreeningManagerV2Service.serviceNames.SSM, scopeSubscriberKey, scopeKey, listItem.listKey, screenableEntry).then(function (response) {
                    if (!_.isUndefined(response) && !_.isUndefined(response.errorCode)) {
                        var text = '';
                        ...
                        listItem['serviceKey'] = ScreeningManagerV2Service.serviceNames.SSM;
                        ...
                        $log.debug('A new item [', listItem.screenableEntryId, ', ', listItem.screenableCorrelator, '] added to ' + ScreeningManagerV2Service.scopes.WELCOME_SMS_BLACKLIST_SCOPE_KEY + ' list.');
                    }
                }, function (response) {
                    $log.debug('Error: ', response);
                });
        ...
        $scope.updateScreeningRule = function (selectedScreeningModeTypes, direction) {
            ScreeningManagerV2Service.updateScreeningRule(ScreeningManagerV2Service.serviceNames.SSM, $scope.offer.name, ScreeningManagerV2Service.scopes.MSISDN_SCOPE_KEY, selectedScreeningModeTypes.msisdn).then(function (response1) {
                ScreeningManagerV2Service.updateScreeningRule(ScreeningManagerV2Service.serviceNames.SSM, $scope.offer.name, ScreeningManagerV2Service.scopes.SAN_SCOPE_KEY, selectedScreeningModeTypes.san).then(function (response2) {
                    notification({
                        type: 'success',
