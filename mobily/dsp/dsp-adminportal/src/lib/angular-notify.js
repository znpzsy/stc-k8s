angular.module('notification', [])
    ...
    .factory('notification', ['$rootScope', '$timeout', '$translate', function ($rootScope, $timeout, $translate) {
        var messages = [];
        ...
        var notification = function (message, timeout) {
            switch (message.type) {
                case 'info':
                    message.text = '<strong>' + $translate.instant('Notifications.Info') + ':</strong>&nbsp;' + message.text;
                    message.fadeOut = {enabled: true, delay: (timeout === undefined ? 7000 : timeout)};
                    break;
                case 'success':
                    message.text = '<strong>' + $translate.instant('Notifications.Success') + ':</strong>&nbsp;' + message.text;
                    message.fadeOut = {enabled: true, delay: (timeout === undefined ? 7000 : timeout)};
                    break;
                case 'warning':
                    message.text = '<strong>' + $translate.instant('Notifications.Warning') + ':</strong>&nbsp;' + message.text;
                    message.fadeOut = {enabled: true, delay: (timeout === undefined ? 10000 : timeout)};
                    break;
                case 'danger':
                    message.text = '<strong>' + $translate.instant('Notifications.Important') + ':</strong>&nbsp;' + message.text;
                    message.fadeOut = {enabled: true, delay: (timeout === undefined ? 10000 : timeout)};
                    break;
                default:
