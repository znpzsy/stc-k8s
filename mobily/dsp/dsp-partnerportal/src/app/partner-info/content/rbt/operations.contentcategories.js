(function () {
    ...
    angular.module('partnerportal.partner-info.operations.rbt.contentcategories', []);

    var PartnerInfoContentManagementContentCategoriesRBTModule = angular.module('partnerportal.partner-info.operations.rbt.contentcategories');

    PartnerInfoContentManagementContentCategoriesRBTModule.config(function ($stateProvider) {

        $stateProvider.state('partner-info.operations.rbt.contentcategories', {
            abstract: true,
        ...
        }).state('partner-info.operations.rbt.contentcategories.list', {
            url: "",
            templateUrl: "partner-info/content/rbt/operations.contentcategories.html",
            controller: 'PartnerInfoContentManagementContentCategoriesRBTCtrl'
        });

        // Create
        $stateProvider.state('partner-info.operations.rbt.contentcategories.new', {
            url: "/new",
            templateUrl: "partner-info/content/rbt/operations.contentcategories.details.html",
            controller: 'PartnerInfoContentManagementContentCategoriesRBTNewCtrl',
            resolve: {
                contentCategory: function () {
                    return null;
                }
            }
        }).state('partner-info.operations.rbt.contentcategories.resendcreatetask', {
            ...
            resolve: {
                contentCategory: function ($stateParams, $q, WorkflowsService) {
                    var deferred = $q.defer();

                    WorkflowsService.getContentCategoryRBT($stateParams.id).then(function (rbtCategoryResponse) {
                        deferred.resolve({
                            category: rbtCategoryResponse.objectDetail
                        });
        ...
        // Update
        $stateProvider.state('partner-info.operations.rbt.contentcategories.update', {
            url: "/update/:id",
            templateUrl: "partner-info/content/rbt/operations.contentcategories.details.html",
            controller: 'PartnerInfoContentManagementContentCategoriesRBTUpdateCtrl',
            resolve: {
                contentCategory: function ($stateParams, ContentManagementService) {
                    return ContentManagementService.getContentCategoryRBT($stateParams.id);
                }
            }
        }).state('partner-info.operations.rbt.contentcategories.resendupdatetask', {
    ...
    PartnerInfoContentManagementContentCategoriesRBTModule.controller('PartnerInfoContentManagementContentCategoriesRBTCommonCtrl', function ($scope, $log, $q, $state, $controller, $filter, UtilService, SessionService, ContentManagementService, CMPFService,
        ...
        $log.debug('PartnerInfoContentManagementContentCategoriesRBTCommonCtrl');
        ...
        $scope.cancel = function () {
            $state.go('partner-info.operations.rbt.contentcategories.list');
        };
    });

    PartnerInfoContentManagementContentCategoriesRBTModule.controller('PartnerInfoContentManagementContentCategoriesRBTCtrl', function ($scope, $log, $controller, $state, $uibModal, $filter, $translate, notification, NgTableParams, NgTableService,
                                                                                                                                        DateTimeConstants, ContentManagementService, WorkflowsService, SessionService, CMPFService,
                                                                                                                                        DEFAULT_REST_QUERY_LIMIT) {
        $log.debug('PartnerInfoContentManagementContentCategoriesRBTCtrl');
        ...
        $scope.exportOptions = {
            columns: [
                {
                    fieldName: 'id',
                    headerKey: 'PartnerInfo.Contents.RBT.ContentCategories.Id'
                },
                {
                    fieldName: 'name',
                    headerKey: 'PartnerInfo.Contents.RBT.ContentMetadatas.ContentInfo.ContentCategory'
                },
                {
                    fieldName: 'subscriptionEnabled',
                    headerKey: 'PartnerInfo.Contents.RBT.ContentCategories.SubscriptionEnabled',
                    filter: {name: 'YesNoFilter'}
                },
                {
                    fieldName: 'promoted',
                    headerKey: 'PartnerInfo.Contents.RBT.ContentCategories.Promoted',
                    filter: {name: 'YesNoFilter'}
                },
                {
                    fieldName: 'totalSubscriptionCount',
                    headerKey: 'PartnerInfo.Contents.RBT.ContentCategories.Subscriptions'
                },
                {
                    fieldName: 'status',
        ...
        $scope.contentCategoryList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.status !== 'WAITING') {
                    ContentManagementService.getContentCategoriesRBTByOrganizationId(filter.page, filter.limit, filter.sortFieldName, filter.sortOrder, filter.statuses, filter.filterText, filter.orgId).then(function (response) {
        ...
        $scope.removeOnWorkflow = function (rbtProviderBusinessTypeProfile, contentCategory) {
            ...
            modalInstance.result.then(function () {
                ...
                WorkflowsService.deleteContentCategoryRBT(contentCategoryItem).then(function (response) {
                    if (response && response.code === 2001) {
                        ...
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.DeleteFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
                    ...
                    if (response && response.data && response.data.message) {
                        ...
                        notification({
                            type: 'warning',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.DeleteFlowError')
                        });
                    }
                });
            }, function () {
    ...
    PartnerInfoContentManagementContentCategoriesRBTModule.controller('PartnerInfoContentManagementContentCategoriesRBTNewCtrl', function ($scope, $log, $controller, $q, $filter, $translate, notification, UtilService, DateTimeConstants,
                                                                                                                                           CMPFService, WorkflowsService, ContentManagementService, contentCategory) {
        $log.debug('PartnerInfoContentManagementContentCategoriesRBTNewCtrl');

        $controller('PartnerInfoContentManagementContentCategoriesRBTCommonCtrl', {
            $scope: $scope
        });
        ...
        $scope.sendToWorkflow = function (rbtProviderBusinessTypeProfile, contentCategory) {
            var contentCategoryItem = {
                "from": {
                    "isAdmin": (rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED'),
                    "userId": $scope.username,
            ...
            WorkflowsService.createContentCategoryRBT(contentCategoryItem).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    $q.all(promises).then(function () {
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.CreateFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
                ...
                if (response && response.data && response.data.message) {
                    ...
                    notification({
                        type: 'warning',
                        text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.CreateFlowError')
                    });
                }
            });
        };
    ...
    PartnerInfoContentManagementContentCategoriesRBTModule.controller('PartnerInfoContentManagementContentCategoriesRBTUpdateCtrl', function ($scope, $log, $controller, $stateParams, $q, $filter, $translate, notification, Restangular, UtilService,
                                                                                                                                              CMPFService, WorkflowsService, DateTimeConstants, FileDownloadService, ContentManagementService,
                                                                                                                                              contentCategory) {
        $log.debug('PartnerInfoContentManagementContentCategoriesRBTUpdateCtrl');
        ...
        $scope.contentCategory.organization = $scope.sessionOrganization;
        ...
        if ($scope.contentCategory.defaultToneId) {
            ContentManagementService.getTone($scope.contentCategory.defaultToneId).then(function (response) {
        ...
        $scope.sendToWorkflow = function (rbtProviderBusinessTypeProfile, contentCategory) {
            var contentCategoryItem = {
                ...
                "organizationId": $scope.originalContentCategory.organizationId,
                "organizationName": $scope.originalContentCategory.organizationName,
                ...
                "status": $scope.originalContentCategory.status,
                // Changed values
                "name": contentCategory.name,
                "launchDate": ($scope.dateHolder.startDate ? $filter('date')($scope.dateHolder.startDate, 'yyyy-MM-dd') + 'T00:00:00' : ''),
                "expireDate": ($scope.dateHolder.endDate ? $filter('date')($scope.dateHolder.endDate, 'yyyy-MM-dd') + 'T00:00:00' : ''),
            ...
            WorkflowsService.updateContentCategoryRBT(contentCategoryItem).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    $q.all(promises).then(function () {
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.UpdateFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
                ...
                if (response && response.data && response.data.message) {
                    ...
                    notification({
                        type: 'warning',
                        text: $translate.instant('PartnerInfo.Contents.RBT.ContentCategories.Messages.UpdateFlowError')
                    });
                }
            });
        };
