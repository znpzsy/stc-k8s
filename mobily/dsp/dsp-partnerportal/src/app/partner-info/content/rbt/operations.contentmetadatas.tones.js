(function () {
    ...
    PartnerInfoContentManagementContentMetadatasRBTTonesModule.config(function ($stateProvider) {
        ...
        }).state('partner-info.operations.rbt.contentmetadatas.tones.list', {
            url: "",
        ...
        $stateProvider.state('partner-info.operations.rbt.contentmetadatas.tones.new', {
        ...
        }).state('partner-info.operations.rbt.contentmetadatas.tones.resendcreatetask', {
            ...
            resolve: {
                tone: function ($stateParams, $q, WorkflowsService) {
                    var deferred = $q.defer();

                    WorkflowsService.getToneRBT($stateParams.id).then(function (toneResponse) {
                        deferred.resolve({
                            tone: toneResponse.objectDetail
                        });
        ...
        $stateProvider.state('partner-info.operations.rbt.contentmetadatas.tones.update', {
            ...
            resolve: {
                tone: function ($stateParams, ContentManagementService) {
                    return ContentManagementService.getTone($stateParams.id);
                }
            }
        }).state('partner-info.operations.rbt.contentmetadatas.tones.resendupdatetask', {
    ...
    PartnerInfoContentManagementContentMetadatasRBTTonesModule.controller('PartnerInfoContentManagementContentMetadatasRBTTonesCommonCtrl', function ($scope, $log, $q, $state, $controller, $filter, UtilService, SessionService, ContentManagementService, CMPFService,
                                                                                                                                                      CMS_RBT_STATUS_TYPES, CMS_GENDERS, CMS_ACCESS_CHANNELS, DURATION_UNITS, CMS_RBT_REV_SHARE_POLICIES_TONE, DEFAULT_REST_QUERY_LIMIT) {
        ...
        $scope.getPrepareRBTSettelementTypes().then(function (settlementTypes) {
    ...
    PartnerInfoContentManagementContentMetadatasRBTTonesModule.controller('PartnerInfoContentManagementContentMetadatasRBTTonesCtrl', function ($rootScope, $scope, $log, $controller, $state, $timeout, $uibModal, $filter, $translate, notification, NgTableParams, NgTableService,
        ...
        $scope.contentMetadataList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.status !== 'WAITING') {
                    ContentManagementService.getTonesByOrganizationId(filter.page, filter.limit, filter.sortFieldName, filter.sortOrder, filter.statuses, filter.filterText, filter.orgId).then(function (response) {
        ...
        $scope.showTaskDetails = function (contentMetadata) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($scope, $controller, $uibModalInstance, moods, artists, albums, contentCategories, taskDetail) {
                ...
                resolve: {
                    ...
                    artists: function ($rootScope, ContentManagementService, DEFAULT_REST_QUERY_LIMIT) {
                        ...
                        return ContentManagementService.getArtistsByOrganizationId(0, DEFAULT_REST_QUERY_LIMIT, organizationId);
                    },
                    albums: function ($rootScope, ContentManagementService, DEFAULT_REST_QUERY_LIMIT) {
                        var organizationId = $rootScope.getOrganizationId();

                        return ContentManagementService.getAlbumsByOrganizationId(0, DEFAULT_REST_QUERY_LIMIT, organizationId);
                    },
            ...
            modalInstance.result.then(function () {
                ...
                WorkflowsService.deleteToneRBT(toneItem).then(function (response) {
                    if (response && response.code === 2001) {
                        ...
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentMetadatas.Tones.Messages.DeleteFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
                    ...
                    if (response && response.data && response.data.message) {
                        ...
                        notification({
                            type: 'warning',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentMetadatas.Tones.Messages.DeleteFlowError')
                        });
                    }
                });
            }, function () {
            ...
            WorkflowsService.createToneRBT(toneItem).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    $q.all(promises).then(function () {
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentMetadatas.Tones.Messages.CreateFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
            ...
            WorkflowsService.updateToneRBT(toneItem).then(function (response) {
                if (response && response.code === 2001) {
                    ...
                    $q.all(promises).then(function () {
                        notification.flash({
                            type: 'success',
                            text: $translate.instant('PartnerInfo.Contents.RBT.ContentMetadatas.Tones.Messages.UpdateFlowStartedSuccessful' + ((rbtProviderBusinessTypeProfile.TrustedStatus === 'TRUSTED') ? 'ForAdmin' : ''))
                        });
