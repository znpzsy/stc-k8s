(function () {
    ...
    angular.module('partnerportal.partner-info.offers.smsportali18nprofile', []);

    var PartnerInfoOffersSMSPortali18nProfileModule = angular.module('partnerportal.partner-info.offers.smsportali18nprofile');

    PartnerInfoOffersSMSPortali18nProfileModule.controller('PartnerInfoOffersSMSPortali18nProfileCtrl', function ($scope, $log, $uibModal, $filter, $translate, CMPFService) {
        $log.debug('PartnerInfoOffersSMSPortali18nProfileCtrl');
        ...
        $scope.addSMSPortali18nProfile = function (offer) {
            var modalInstance = $uibModal.open({
                templateUrl: 'partner-info/offers/operations.offers.modal.smsportali18nprofile.html',
                    ...
                    $scope.$watch('smsPortali18nProfile.SubCommands', function (newVal, oldVal) {
                        if ($scope.form) {
                            ...
                            if (unsubOtherCommandIntersection.length > 0) {
                                ...
                                $scope.subCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.SubOtherCommandAlreadyUsed', {
                                    ...
                                    _.each(offer.smsPortali18nProfileList, function (smsPortali18nProfile) {
                                        if ($scope.smsPortali18nProfile.SubUnsubShortCode === smsPortali18nProfile.SubUnsubShortCode) {
                                            ...
                                            if (subCommandIntersection.length > 0 || unsubCommandIntersection.length > 0) {
                                                $scope.form.SubCommands.$setValidity('availabilityCheck', false);
                                                $scope.subCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.SubThisCommandAlreadyUsed', {
                                    ...
                                    _.each($scope.offersOfPartner, function (offer) {
                                        _.each(offer.smsPortali18nProfiles, function (smsPortali18nProfile) {
                                            if ($scope.smsPortali18nProfile.SubUnsubShortCode === smsPortali18nProfile.SubUnsubShortCode) {
                                                ...
                                                if (subCommandIntersection.length > 0 || unsubCommandIntersection.length > 0) {
                                                    $scope.form.SubCommands.$setValidity('availabilityCheck', false);
                                                    $scope.subCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.SubCommandAlreadyUsed', {
                                                        command: subCommandIntersection.length > 0 ? subCommandIntersection.join(';') : unsubCommandIntersection.join(';'),
                                        ...
                                        _.each(service.onDemandi18nProfiles, function (onDemandi18nProfile) {
                                            if ($scope.smsPortali18nProfile.SubUnsubShortCode === onDemandi18nProfile.OnDemandShortCode) {
                                                ...
                                                if (onDemandCommandIntersection.length > 0) {
                                                    $scope.form.SubCommands.$setValidity('availabilityCheck', false);
                                                    $scope.subCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.SubCommandAlreadyUsedByService', {
                    ...
                    $scope.$watch('smsPortali18nProfile.UnsubCommands', function (newVal, oldVal) {
                        if ($scope.form) {
                            ...
                            if (subOtherCommandIntersection.length > 0) {
                                ...
                                $scope.unsubCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.UnsubOtherCommandAlreadyUsed', {
                                    ...
                                    _.each(offer.smsPortali18nProfileList, function (smsPortali18nProfile) {
                                        if ($scope.smsPortali18nProfile.SubUnsubShortCode === smsPortali18nProfile.SubUnsubShortCode) {
                                            ...
                                            if (unsubCommandIntersection.length > 0 || subCommandIntersection.length > 0) {
                                                $scope.form.UnsubCommands.$setValidity('availabilityCheck', false);
                                                $scope.unsubCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.UnsubThisCommandAlreadyUsed', {
                                    ...
                                    _.each($scope.offersOfPartner, function (offer) {
                                        _.each(offer.smsPortali18nProfiles, function (smsPortali18nProfile) {
                                            if ($scope.smsPortali18nProfile.SubUnsubShortCode === smsPortali18nProfile.SubUnsubShortCode) {
                                                ...
                                                if (unsubCommandIntersection.length > 0 || subCommandIntersection.length > 0) {
                                                    $scope.form.UnsubCommands.$setValidity('availabilityCheck', false);
                                                    $scope.unsubCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.UnsubCommandAlreadyUsed', {
                                                        command: unsubCommandIntersection.length > 0 ? unsubCommandIntersection.join(';') : subCommandIntersection.join(';'),
                                                        offerName: offer.name
                                                    });
                                    ...
                                    _.each($scope.servicesOfPartner, function (service) {
                                        _.each(service.onDemandi18nProfiles, function (onDemandi18nProfile) {
                                            if ($scope.smsPortali18nProfile.SubUnsubShortCode === onDemandi18nProfile.OnDemandShortCode) {
                                                ...
                                                if (onDemandCommandIntersection.length > 0) {
                                                    $scope.form.UnsubCommands.$setValidity('availabilityCheck', false);
                                                    $scope.unsubCommandsAvailabilityMessage = $translate.instant('PartnerInfo.Offers.SMSPortali18nProfile.UnsubCommandAlreadyUsedByService', {
                                                        command: onDemandCommandIntersection.join(';'),
                                                        serviceName: service.name
                                                    });
                ...
                resolve: {
                    ...
                    offersOfPartner: function ($rootScope, CMPFService, DEFAULT_REST_QUERY_LIMIT) {
                        var organizationId = $rootScope.getOrganizationId();

                        return CMPFService.getOffersByOrganizationId(organizationId, false, true, true, null, [CMPFService.SMS_PORTAL_I18N_PROFILE]);
                    },
