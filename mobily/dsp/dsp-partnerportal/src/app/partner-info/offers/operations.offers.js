(function () {
    ...
    angular.module('partnerportal.partner-info.offers', [
        'partnerportal.partner-info.offers.new',
        'partnerportal.partner-info.offers.update',
        'partnerportal.partner-info.offers.smsportali18nprofile'
    ]);

    var PartnerInfoOffersModule = angular.module('partnerportal.partner-info.offers');

    PartnerInfoOffersModule.config(function ($stateProvider) {

        $stateProvider.state('partner-info.offers', {
            abstract: true,
            url: "/offers",
        ...
        }).state('partner-info.offers.list', {
            url: "",
            templateUrl: "partner-info/offers/operations.offers.html",
            controller: 'PartnerInfoOffersCtrl',
            resolve: {
    ...
    PartnerInfoOffersModule.controller('PartnerInfoOffersCommonCtrl', function ($scope, $log, $state, $filter, $uibModal, $controller, Restangular, CMPFService, SessionService, UtilService,
                                                                                shortCodesOrganization, businessTypesOrganization, settlementTypesOrganization, serviceCategoriesOrganization,
                                                                                DURATION_UNITS, OFFER_CHARGING_POLICIES, OFFER_HANDLERS, OFFER_INITIAL_CHARGING_POLICIES,
                                                                                OFFER_CHARGING_FAILURE_POLICIES, OFFER_CHARGE_ONS, OFFER_TERMINATION_POLICIES, OFFER_RENEWAL_POLICIES,
                                                                                OFFER_BLACK_LISTED_SUBSCRIBER_POLICIES, OFFER_PACKAGE_ELIGIBILITIES, packageList) {
        $log.debug('PartnerInfoOffersCommonCtrl');
        ...
        $scope.showServices = function () {
            var modalInstance = $uibModal.open({
                templateUrl: 'partner-info/offers/operations.offers.modal.services.html',
                controller: 'PartnerInfoOffersServicesModalInstanceCtrl',
                size: 'lg',
                resolve: {
                    ...
                    services: function ($rootScope, $q, CMPFService, DEFAULT_REST_QUERY_LIMIT) {
                        var organizationId = $rootScope.getOrganizationId();

                        var deferred = $q.defer();

                        CMPFService.getServicesByOrganizationId(organizationId, false, false, 'ACTIVE', null).then(function (services) {
                            /*
    ...
    PartnerInfoOffersModule.controller('PartnerInfoOffersCtrl', function ($scope, $log, $filter, $state, $uibModal, $translate, notification, NgTableParams, NgTableService, Restangular,
                                                                          SessionService, WorkflowsService, CMPFService, offers, DEFAULT_REST_QUERY_LIMIT) {
        $log.debug('PartnerInfoOffersCtrl');
                        ...
                        if (waitingOfferTasks && waitingOfferTasks.length > 0) {
                            _.each(waitingOfferTasks, function (offerTask) {
                                if (offerTask && offerTask.name && (offerTask.name.toLowerCase() === 'offer create task')) {
                                    ...
                                    $scope.offers.list.push(offerTask.objectDetail);
                                }
                            });
                        }

                        $scope.offers.tableParams.page(1);
                        $scope.offers.tableParams.reload();
                    });
                } else {
                    $scope.offers.list = _.where($scope.originalOfferList, {state: state});
                }
            } else {
                $scope.offers.list = angular.copy($scope.originalOfferList);
            }
        ...
        offerList = $filter('orderBy')(offerList, 'id');
        $scope.originalOfferList = angular.copy(offerList);

        $scope.exportAllData = function (fileNamePrefix, exporter) {
            CMPFService.getOffersByOrganizationId(SessionService.getSessionOrganization().id, true, true, null, [CMPFService.XSM_OFFER_PROFILE]).then(function (exportingOffers) {
        ...
        $scope.exportOptions = {
            columns: [
                ...
                },
                {
                    ...
                    headerKey: 'PartnerInfo.Offers.XsmOfferProfile.Description'
                },
                {
                    fieldName: 'serviceNames',
                    headerKey: 'PartnerInfo.Offers.Services'
                },
                {
                    fieldName: 'state',
        ...
        $scope.offers.tableParams = new NgTableParams({
            page: 1,
            count: 10,
            sorting: {
                "id": 'asc'
            ...
            getData: function ($defer, params) {
                ...
                var filteredListData = NgTableService.filterList(filterText, filterColumns, $scope.offers.list);
                var orderedData = params.sorting() ? $filter('orderBy')(filteredListData, params.orderBy()) : $scope.offers.list;
        ...
        $scope.filterTable = _.debounce(function (filterText, filterColumns) {
            $scope.offers.tableParams.settings().$scope.filterText = filterText;
            $scope.offers.tableParams.settings().$scope.filterColumns = filterColumns;
            $scope.offers.tableParams.page(1);
            $scope.offers.tableParams.reload();
        }, 500);
        ...
        $scope.remove = function (offer) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($scope, $uibModalInstance, $translate, $controller, $sce, offer) {
                ...
                resolve: {
                    offer: function (CMPFService) {
                        return CMPFService.getOffer(offer.id, true, true);
                    }
                }
            });

            modalInstance.result.then(function () {
                $log.debug('Trying to remove offer: ', currentSDPOffer);
                ...
                var offerItem = {
                    ...
                    "offerDetail": {
                        "offerId": currentSDPOffer.id,
                        "name": currentSDPOffer.name,
                        "offerProviderId": currentSDPOffer.organizationId,
                        "state": currentSDPOffer.state,
                        // Changed attributes.
                        // ...
                    }
                };
                ...
                /*
                // Offer deletion workflow is starting.
                WorkflowsService.deleteOffer(offerItem).then(function (createResponse) {
                    $log.debug('Offer Deleted Successfully. Response: ', createResponse);
                    ...
                    $log.debug('Offer cannot be deleted. Error: ', createErrorResponse);
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($scope, $uibModalInstance, $translate, $controller, $sce) {
                    ...
                    if (newState === 'SUSPENDED') {
                        message = $translate.instant('PartnerInfo.Offers.Messages.SuspendConfirmationMessage');
                    } else if (newState === 'ACTIVE') {
                        message = $translate.instant('PartnerInfo.Offers.Messages.ActivateConfirmationMessage');
                    } else if (newState === 'INACTIVE') {
                        message = $translate.instant('PartnerInfo.Offers.Messages.InactivateConfirmationMessage');
                    } else if (newState === 'HIDDEN') {
                        message = $translate.instant('PartnerInfo.Offers.Messages.HideConfirmationMessage');
                    } else if (newState === 'UNHIDDEN') {
                        message = $translate.instant('PartnerInfo.Offers.Messages.UnhideConfirmationMessage');
                        newState = 'ACTIVE';
                    }
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    modalTitleKey: function () {
                        return 'PartnerInfo.Offers.ServicesModalTitle';
                    },
    ...
    PartnerInfoOffersModule.controller('PartnerInfoOffersServicesModalInstanceCtrl', function ($scope, $uibModalInstance, $log, $filter, NgTableParams, NgTableService, Restangular,
                                                                                               CMPFService, servicesParameter, offerNameParameter, services) {
        $log.debug('PartnerInfoOffersServicesModalInstanceCtrl');
