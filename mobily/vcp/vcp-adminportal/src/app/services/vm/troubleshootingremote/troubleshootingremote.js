(function () {
    ...
    angular.module('adminportal.services.vm.troubleshootingremote', []);

    var VMTroubleshootingRemoteModule = angular.module('adminportal.services.vm.troubleshootingremote');

    VMTroubleshootingRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('services.vm.troubleshootingremote', {
            url: "/troubleshooting-remote",
            templateUrl: "services/vm/troubleshooting/troubleshooting.html",
            controller: 'VMTroubleshootingRemoteCtrl'
        });

    });

    VMTroubleshootingRemoteModule.controller('VMTroubleshootingRemoteCtrl', function ($scope, $log, $controller, $timeout, $filter, DateTimeConstants) {
        $log.debug('VMTroubleshootingRemoteCtrl');
        ...
        $controller('VMTroubleshootingRemoteTableCtrl', {$scope: $scope});
        $controller('VMTroubleshootingRemoteHistoryCtrl', {$scope: $scope});
    });

    VMTroubleshootingRemoteModule.controller('VMTroubleshootingRemoteTableCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams,
                                                                                           UtilService, GeneralESService, DateTimeConstants) {
        $log.debug('VMTroubleshootingRemoteTableCtrl');

        $scope.exportFileName = 'VoiceMailServiceTroubleshootingRecordsRemote';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.askService) {
                    GeneralESService.findVMHistoryRemote(filter, additionalFilterFields).then(function (response) {
        ...
        $scope.showDetails = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'services/vm/troubleshooting/troubleshooting.edr.details.html',
    ...
    VMTroubleshootingRemoteModule.controller('VMTroubleshootingRemoteHistoryCtrl', function ($scope, $log, $filter, notification, $translate, $uibModal, NgTableParams,
                                                                                             GeneralESService, DateTimeConstants) {
        $log.debug('VMTroubleshootingRemoteHistoryCtrl');
        ...
        $scope.showVMHistory = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'services/vm/troubleshooting/troubleshooting.edr.history.html',
                ...
                resolve: {
                    vmEdrs: function (GeneralESService) {
                        return GeneralESService.findVMDetailedHistoryRemote(edrRecord.sessionid);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
