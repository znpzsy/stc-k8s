(function () {
    ...
    angular.module('adminportal.products.ussi.troubleshootingremote', []);

    var USCTroubleshootingRemoteModule = angular.module('adminportal.products.ussi.troubleshootingremote');

    USCTroubleshootingRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('products.ussi.troubleshootingremote', {
            url: "/troubleshooting-remote",
            templateUrl: "products/ussi/troubleshooting/troubleshooting.html",
            controller: 'USSITroubleshootingRemoteCtrl'
        });

    });

    USCTroubleshootingRemoteModule.controller('USSITroubleshootingRemoteCtrl', function ($scope, $log, $controller, $timeout, $filter, DateTimeConstants, USC_POINT_TYPES) {
        ...
        $controller('USSITroubleshootingRemoteTableCtrl', {$scope: $scope});
    });

    USCTroubleshootingRemoteModule.controller('USSITroubleshootingRemoteTableCtrl', function ($scope, $log, $q, $filter, UtilService, NgTableParams, notification, $translate,
                                                                                              GeneralESService, DateTimeConstants, $uibModal) {
        $scope.exportFileName = 'USSIServiceCenterTroubleshootingRecordsRemote';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.askService) {
                    GeneralESService.findUSSIGatewayHistoryInitSessionsRemote(filter, additionalFilterFields).then(function (response) {
        ...
        $scope.showHistory = function (edrRecord) {
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    ...
                    edrHistory: function () {
                        return GeneralESService.findUSSIGatewayCenterHistoryRemote(edrRecord.ticket);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
