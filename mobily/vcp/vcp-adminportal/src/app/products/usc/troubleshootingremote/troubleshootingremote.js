(function () {
    ...
    angular.module('adminportal.products.usc.troubleshootingremote', []);

    var USCTroubleshootingRemoteModule = angular.module('adminportal.products.usc.troubleshootingremote');

    USCTroubleshootingRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('products.usc.troubleshootingremote', {
            url: "/troubleshooting-remote",
            templateUrl: "products/usc/troubleshooting/troubleshooting.html",
            controller: 'USCTroubleshootingRemoteCtrl'
        });

    });

    USCTroubleshootingRemoteModule.controller('USCTroubleshootingRemoteCtrl', function ($scope, $log, $controller, $timeout, $filter, DateTimeConstants, USC_POINT_TYPES) {
        ...
        $controller('USCTroubleshootingRemoteTableCtrl', {$scope: $scope});
    });

    USCTroubleshootingRemoteModule.controller('USCTroubleshootingRemoteTableCtrl', function ($scope, $log, $q, $filter, UtilService, NgTableParams, notification, $translate,
                                                                                             GeneralESService, DateTimeConstants, $uibModal) {
        $scope.exportFileName = 'USSDServiceCenterTroubleshootingRecordsRemote';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                if (params.settings().$scope.askService) {
                    GeneralESService.findUSSDServiceCenterHistoryInitSessionsRemote(filter, additionalFilterFields).then(function (response) {
        ...
        $scope.viewContent = function (edrRecord) {
            $uibModal.open({
                animation: false,
                templateUrl: 'products/usc/troubleshooting/troubleshooting.edr.content.html',
                ...
                resolve: {
                    ...
                    edrRecordContent: function () {
                        return GeneralESService.findUSSDServiceCenterHistoryDetailRemote(edrRecord.sessionId, edrRecord.application, edrRecord.eventtype, edrRecord.timestamp);
                    }
                }
            });
        };
        ...
        $scope.showHistory = function (edrRecord) {
            ...
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/usc/troubleshooting/troubleshooting.edr.history.html',
                size: 'xlg',
                resolve: {
                    ...
                    edrHistory: function () {
                        return GeneralESService.findUSSDServiceCenterHistoryAllRemote(edrRecord.sessionId);
                    },
                    edrHistoryContents: function () {
                        return GeneralESService.findUSSDServiceCenterHistoryDetailAllRemote(edrRecord.sessionId);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
