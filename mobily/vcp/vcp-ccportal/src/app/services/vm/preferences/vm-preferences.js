(function () {
    ...
    VMPreferencesModule.config(function ($stateProvider) {
        ...
        $stateProvider.state('services.vm.preferences', {
            ...
            resolve: {
                searchPreferences: function ($rootScope, $q, UtilService, VMProvisioningService) {
    ...
    VMPreferencesModule.controller('VMPreferencesCtrl', function ($scope, $rootScope, $log, $state, $uibModal, notification, $translate, Restangular, UtilService, VMProvisioningService,
                                                                  SUBSCRIBER_LANGUAGES, preferences, classOfServiceProfiles, updateMethod) {
        $log.debug('VMPreferencesCtrl');

        if (!$rootScope.isSubscriberActive()) {
            $scope.$parent.$parent.error_message = 'CommonMessages.SubscriberIsNotActive';
            return;
        }
