(function () {
    ...
    angular.module('ccportal.services.mca.activity-history-remote', []);

    var MCAActivityHistoryRemoteModule = angular.module('ccportal.services.mca.activity-history-remote');

    MCAActivityHistoryRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('services.mca.activity-history-remote', {
            url: "/activity-history-remote",
            templateUrl: "services/mca/activity-history/mca-activity-history.html",
            controller: 'MCAActivityHistoryRemoteCtrl',
            resolve: {}
        });

    });

    MCAActivityHistoryRemoteModule.controller('MCAActivityHistoryRemoteCtrl', function ($scope, $log, $controller, $timeout, $filter, UtilService, DateTimeConstants) {
        ...
        $controller('MCAActivityHistoryRemoteTableCtrl', {$scope: $scope});
    });

    MCAActivityHistoryRemoteModule.controller('MCAActivityHistoryRemoteTableCtrl', function ($scope, $log, $q, UtilService, NgTableParams, notification, $translate,
                                                                                             GeneralESService, DateTimeConstants) {
        $scope.exportFileName = 'MissedCallNotificationServiceActivityHistoryRecordsRemote';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                GeneralESService.findMCAHistoryRemote(filter, additionalFilterFields).then(function (response) {
