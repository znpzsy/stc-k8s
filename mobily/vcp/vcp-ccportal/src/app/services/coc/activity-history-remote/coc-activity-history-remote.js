(function () {
    ...
    angular.module('ccportal.services.coc.activity-history-remote', []);

    var COCActivityHistoryRemoteModule = angular.module('ccportal.services.coc.activity-history-remote');

    COCActivityHistoryRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('services.coc.activity-history-remote', {
            url: "/activity-history-remote",
            templateUrl: "services/coc/activity-history/coc-activity-history.html",
            controller: 'COCActivityHistoryRemoteCtrl',
            resolve: {}
        });

    });

    COCActivityHistoryRemoteModule.controller('COCActivityHistoryRemoteCtrl', function ($scope, $log, $controller, $timeout, $filter, UtilService, DateTimeConstants) {
        ...
        $controller('COCActivityHistoryRemoteTableCtrl', {$scope: $scope});
    });

    COCActivityHistoryRemoteModule.controller('COCActivityHistoryRemoteTableCtrl', function ($scope, $log, $q, UtilService, NgTableParams, notification, $translate,
                                                                                             GeneralESService, DateTimeConstants) {
        $scope.exportFileName = 'CollectCallServiceActivityHistoryRecordsRemote';
        ...
        $scope.activityHistory.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                GeneralESService.findCCHistoryRemote(filter, additionalFilterFields).then(function (response) {
